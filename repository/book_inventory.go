...
func (r *pgBookRepository) BulkUpsertBookTitles(ctx context.Context, titles []BookTitle) (int, error) {
	...
	// Lernmittel, Fach und Jahrgangsspanne (Migration 093, seit 135 die eine Jahrgangsangabe): Der Import liest sie aus Litteras
	...
	const qInsert = `
		...
		VALUES ($1, $2, NULLIF($3, ''), $4, NULLIF($5, 0), NULLIF($6, ''),
		        $7, NULLIF($8, ''), COALESCE(NULLIF($9, 0), 5), COALESCE(NULLIF($10, 0), 10),
		        CURRENT_TIMESTAMP)
	`
	...
	// nicht kennt. Das Fach füllt nur Leerstellen. Die Jahrgangsspanne
	...
	const qUpdate = `
		UPDATE buecher_titel SET
			...
			jahrgang_von = CASE WHEN $10 > 0 THEN $10 ELSE jahrgang_von END,
			jahrgang_bis = CASE WHEN $11 > 0 THEN $11 ELSE jahrgang_bis END,
