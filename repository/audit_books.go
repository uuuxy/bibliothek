...
func (r *pgAuditRepository) DeleteCopy(ctx context.Context, copyID string, bearbeiterID string) error {
	...
	tag, err := tx.Exec(ctx, "UPDATE buecher_exemplare SET ist_ausgesondert = true, ist_ausleihbar = false, aussonderung_grund = 'AUSSORTIERT', zustand_notiz = 'Systematisch gelöscht', bestellstatus = NULL, letzte_bewegung_am = CURRENT_TIMESTAMP WHERE id = $1 AND ist_ausgesondert = false", copyID)
	if err != nil {
		return err
	}
	if tag.RowsAffected() == 0 {
