...
| Ausweisnummer eindeutig **über Schüler und Kollegium** — die Theke sucht bei jeder Nummer unter beiden; gelöschte Schüler geben die Nummer frei (Migration 118) | 🟢 Trigger mit Sperre je Nummer + PG-Tests über Datenbank und Handler | `trg_schueler_ausweis_eindeutig`, `trg_benutzer_ausweis_eindeutig`, `db/constraints_ausweis_ueber_personen_pg_test.go`, `api/ausweis_ueber_personen_pg_test.go` |
...
| **Wer als Lehrkraft ausleiht, entscheidet die Personenart** (Lehrkraft, LiV), nicht die Rolle — Theke, Buch- und Geräteausleihe, Nachbuchen über eine Bedingung; ein Kollegiumskonto hat immer eine Personenart (Migrationen 119, 120) | 🟢 `SQLAktiveLehrkraft` + Trigger + PG-Tests | `repository/user.go`, `trg_benutzer_kollegium_personenart`, `repository/lehrkraft_an_personenart_pg_test.go`, `db/constraints_personenart_pg_test.go` |
