...
| Jede Bewegung eines Exemplars (Ausleihe, Rückgabe, Rückholen, Aussonderung) setzt `letzte_bewegung_am`, und der Stempel läuft nie rückwärts (auch nicht mit früherem Scan-Zeitpunkt oder aus einer früher begonnenen Transaktion); jede Ausleihe trägt `erfasst_am` (Scan-Zeitpunkt, höchstens Serverzeit) | 🟡 von den Schreibern gesetzt (`sqlStempelVor`/`sqlStempelJetzt`), nicht vom Trigger; PG-Test je Bewegung und für beide Rückwärts-Wege | `migrations/116_bewegungsstempel.sql`, `repository/loan.go`, `repository/bewegungsstempel_pg_test.go`  |
