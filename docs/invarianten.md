...
| Gesperrte/manuell blockierte Schüler leihen nicht                                                                                                                                                                                                              | 🟡 mit Override + Audit                                                                   | `internal/service/loan_checkout_validation.go` (`pruefeGesperrt`, `pruefeManuellGesperrt`) |
| Überfällig-Automatik: ≥ `MaxOverdueItems` sperrt — nur beim Buch der Schülerbücherei; ein Lernmittel kennt keine automatische Sperre (Antwort der Schule vom 22.09.2026) | 🟡 | `internal/service/loan_checkout_validation.go` (`pruefeUeberfaellig`, `pruefeSchuelerAusleihbarMit`) |
...
| Leser, die keine Schüler sind (`art` ≠ `schueler`): Dauerleihe (`ist_handapparat`) mit gespeicherter Frist von einem Jahr; für sie gelten weder Ausleih-Limit noch Sperrprüfung, und die Überfällig-Zählung lässt Dauerleihen aus (Entscheidung 16.09.2026)    | 🟡 Unit-Tests, Abdeckung 93,8 % (23.09.2026)                                              | `internal/service/loan_checkout_validation.go` (`resolveBorrowerAndDueTime`, `zaehleUeberfaelligeMedien`), `loan_checkout_cases.go` (`erzeugeAusleihe`) |
