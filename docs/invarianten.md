...
| Überfällig-Automatik: ≥ `MaxOverdueItems` sperrt                                                                                                                                                                                                               | 🟡                                                                                        | `internal/service/loan_checkout_validation.go` (`pruefeUeberfaellig`)          |
...
| **Sperrreihenfolge** Schüler → Ausleihe → Exemplar (seit 15.09.2026, Stufe 2): Wer Zeilen sperrt, sperrt zuerst `schueler`, dann `ausleihen`, dann `buecher_exemplare` — Online-Scan und Nachbuchen gleich, sonst verklemmen sie sich gegeneinander            | 🟡 Konvention; jeder Schreibpfad hält sie, ein Gate gibt es nicht                         | `internal/service/loan_checkout.go` (`zaehleAktiveSchuelerAusleihen`), `repository/loan.go` (`GetActiveLoanByCopyIDTx`, `StempleBewegungZum`) |
