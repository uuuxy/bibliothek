...
| **Offline-Warteschlange greift für 0,16 % des Bestands** (Theke (B), 13.09.2026, auf dem Server lesend gezählt). Offline reiht die Theke nur Scans mit `B-` ein (`speichereOfflineAktion` in `frontend/src/lib/stores/omnibox.svelte.js`). Auf dem Server tragen 55 von rund 34.800 nicht ausgesonderten Exemplaren diese Form; 30.658 sind Littera-Mediennummern (nur Ziffern), 4.065 Lernmittel (`LMF-…`). Jeder andere Offline-Scan meldet „Netzwerkfehler" und wird nicht gespeichert. Ausweise auf dem Server: Schüler `S-`, Geräte `G-` — keine Überschneidung mit den Buchformen. Dazu, aus dem Code gelesen und nicht nachgestellt: Ein offline gescannter Schülerausweis wird nicht geladen, und die folgenden Bücher reihen sich als Ausleihe an den vorher geladenen Schüler. Einen Offline-Weg für Ausweise gab es nie (keine lokale Schülerliste, kein Service Worker, kein E2E mit `setOffline`); FACHKONZEPT §18.4 verspricht aber, dass die Theke bei Netzausfall „Scans" sammelt, und Peter nennt Weiterarbeiten beim Verbindungsabbruch eine Grundvoraussetzung (13.09.2026). **Entschieden 13.09.2026:** Offline werden alle Buchformen des Bestands gespeichert (`B-`, nur Ziffern, `LMF-`). **Entschieden 13.09.2026:** Ein offline gescannter Schülerausweis kommt in die Warteschlange; die folgenden Bücher werden ihm zugeordnet, der Server löst ihn beim Nachsenden auf. Keine Schülerdaten auf dem Rechner; Sperren prüft der Server erst beim Nachsenden und meldet Abgelehntes mit Barcode (die lokale Schülerliste ist verworfen). | — (entschieden, in Umsetzung) |
| **Neuladen ohne Netz führt zur Anmeldemaske** (13.09.2026, beim Offline-Durchgang am Code gelesen). Der Service Worker (`vite-plugin-pwa`, `registerSW` in `frontend/src/main.js`) hält die Oberfläche vor, API-Antworten aber ausdrücklich nicht (`navigateFallbackDenylist` mit `/api/`, kein `runtimeCaching`). Beim Start fragt `authStore.restoreSession` den Server (`/api/auth/me`, `frontend/src/lib/stores/authStore.svelte.js:115–127`); ohne Antwort erscheint bewusst die Anmeldung, und die geht nur mit Server. Weiterarbeiten ohne Netz gilt damit, solange niemand die Seite neu lädt oder den Rechner neu startet. Die Warteschlange bleibt erhalten und wird nach der nächsten Anmeldung mit Netz nachgebucht. | Soll die Theke nach einem Neuladen ohne Netz weiterarbeiten können (dann bräuchte es eine Anmeldung ohne Server — eine eigene Sicherheitsfrage), oder bleibt die Anmeldemaske der Rückfall? |
| **Offline weiterarbeiten — vier Folgefragen** (13.09.2026, beim Offline-Durchgang am Code bestätigt). (a) Nach 25 s ohne Server-Signal legt `App.svelte` das Vollbild „VERBINDUNG VERLOREN" (`fixed inset-0 z-50`, `backdrop-blur`) über die Theke. (b) Mit geladener Lehrkraft reiht ein Offline-Buchscan eine Rückgabe ein (`speichereOfflineAktion` prüft nur `activeStudent`). (c) Beim Nachbuchen gilt jede fehlerfreie Antwort als Erfolg (`api/action.go`, `Success: err == nil`), und ein zweiter Scan desselben Buchs am selben Schüler wird zur Rückgabe (`loan_checkout.go`, `istEigeneRueckgabe`); lag das Buch noch bei jemand anderem, wird dort nur zurückgenommen. (d) Ein nicht erreichbares Backend bei stehendem Netz (502/503/504) gilt heute nicht als offline. **Entschieden 13.09.2026 (Peter):** (a) An der Theke ein nicht blockierendes Band statt des Vollbilds, Scannen geht weiter. (b) Lehrerausweis und geladene Lehrkraft werden offline wie ein Schülerausweis zugeordnet, die Bücher gehen beim Nachbuchen als Handapparat an die Lehrkraft. (c) Beim Nachbuchen wird die Wirklichkeit gebucht: lag das Buch noch bei jemand anderem, dort zurücknehmen und neu ausleihen; beim selben Schüler nichts umkehren; beide Fälle im Nachbuch-Bericht, nichts verschwindet still. (d) 502/503/504 gelten wie ein Netzausfall. | — (entschieden, in Umsetzung) |
