...
| Kein Weg zum Klassensatz-Ausweisdruck                             | „Klassensatz drucken" (`ClassPrintStation`) wurde am 26.06.2026 aus der Schülerdatei entfernt — Commit-Text: „gehört ins DruckCenter". Dort ist es nie angekommen, die Datei fiel später als toter Code weg (`28b3addd`). Die Funktion lebt: Schülerdatei → nach Klasse suchen → Kopf-Häkchen → „Ausweise drucken". Nur findet sie niemand, der den alten Namen sucht. Zu entscheiden ist der Einstieg, nicht die Funktion. |
| Eingehende Anliegen melden sich nirgends                          | Wunsch/Meldung einer Lehrkraft landet im dritten Reiter unter Bestellungen. `wartendeArbeit()` in `Sidebar.svelte` kennt nur Klassensatz-Reservierungen und offene Etiketten — für offene Anliegen zählt niemand. Wer nicht ohnehin bestellen will, sieht sie nie. Braucht einen Zähl-Endpunkt wie `/api/reservierungen/klassensatz/anzahl`.                                                                                |
| Zwei Ansichten, beide beschriften „Klasse 5"                      | Medienkatalog → „Jahrgänge" gruppiert TITEL nach `gradeLevel`/`track` aus den Buch-Metadaten; Verwaltung → „Schulklassen" zeigt die ECHTEN Klassensätze aus `/api/class-books`. Die Dopplung ist seit dem 08.08.2026 weg (der dritte Reiter), die Namenskollision nicht. Kostet jeden Neuen eine Rückfrage.                                                                                                                 |
...
| Mitarbeiter sehen seit 24.08.2026 „Mahnwesen"                     | Folge der Korrektur des Menüpunkts auf `view_students` (die Routen verlangten es schon immer). Damit erreichen Mitarbeiter per Oberfläche Mahnstufen-Druck und Mahn-Mail-Versand — der Server erlaubte beides vorher schon. Entscheidung: bleibt so (Vorgabe) oder Mahnwesen bekommt ein eigenes Recht.                                                                                                                     |
