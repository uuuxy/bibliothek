...
- **Prüflauf 13.09.2026 über Issues, Register und Dokumentation** (Peter: „überprüfe alles
  ...
  - **Werkzeuge (B):** `scripts/api_inventar.sh` liest `_test.go` mit — `docs/api_inventar.md`
    führt Test-Routen (`GET /api/dinge/{id}`, `POST /extend/`); `security-scan.sh` zielt auf
    `host.docker.internal:8080`, dort läuft kein Stack (Swagger gibt es nur lokal auf 8084).
