...
| A7 | **Aufbewahrung der übrigen Tabellen.** Drei Tabellen hatten keinen Löschpfad. `lehrer_anliegen` (Freitext, Klasse, Name der Lehrkraft über den Fremdschlüssel) bekommt eine Frist: erledigte Anliegen werden nach 365 Tagen gelöscht, einstellbar unter „Datenschutz & Sitzung", 0 = aus; offene Anliegen haben keine Frist. `inventur_sessions` und `bestellungen_verlauf` bleiben **bewusst** unbefristet — die eine ist der Nachweis der Bestandskartei (HKM-Leitfaden LMF 11.3: Ausleihe UND Rücklauf), die andere ein Belegwesen mit eigener Aufbewahrungspflicht; beide tragen keine Schülerdaten. | `jobs/cron_dsgvo_anliegen.go`, PG-Gate `cron_dsgvo_anliegen_pg_test.go` (zwei Rückbauten rot gesehen) | **erledigt 23.08.2026** |
| A6 | `SENTRY_DSN` in Produktion **leer lassen** (sonst Fehlerreports in die USA) oder EU-Instanz; S3-Offsite nur EU/Schulträger. | `.env.example` (`SENTRY_DSN`, `S3_ENDPOINT`), `jobs/backup.go` (`uploadBackupToS3`) | Betriebsregel |
