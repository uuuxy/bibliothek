...
- **Eine Jahrgangsangabe am Titel (22.09.2026, Migration 135):** Die Spanne `jahrgang_von/bis` („Geeignet für Jahrgang von … bis", bei Lernmitteln „Im Unterricht von Jahrgang … bis") ist die eine Angabe; „Klasse" (`grade_level`) ist gefallen. Sie sagte mit denselben Zahlen fast dasselbe, stand in der Maske mit der Vorgabe 5 und hatte außer dem Portal-Filter keinen Leser — die Spanne lesen Mahnwesen, Klassen-Inventur, Buchakte und Portal-Filter. Die Migration übernimmt eine Klasse 6–13 als Spanne dieses Jahrgangs, wo die Spanne noch die Vorgabe 5–10 trug; Klasse 5 ist die Vorgabe der Maske und keine Aussage; eine gepflegte Spanne gewinnt. Ein einzelner Jahrgang ist „von 7 bis 7"; der Scanner-Dialog und der Listenimport (Spalte „klasse") schreiben ihn so, ohne Angabe bleibt die Vorgabe der Datenbank. Die Oberfläche zeigt die Vorgabe 5–10 nirgends als Zahl (`jahrgangSpanne` in `frontend/src/lib/utils/format.js`, dieselbe Regel wie `jahrgangText` im Lernmittel-PDF). Belegt: `inventur/klasse_in_spanne_pg_test.go`.
...
## 17. Einstellungen — die 14 Kategorien
...
| 7   | **Schadensersatz**          | Angaben für den Schadensersatz-Bescheid: Nummer des Schulamtsbereichs und Schulnummer (Teil der Referenznummer), Aufsichtsbehörde (Name und Anschrift), Name der Schulleitung, Geschäftszeichen, Bearbeiter, Durchwahl (freiwillig), Zahlstelle und Bankverbindung (vorbelegt aus dem Musterschreiben), Zahlungsfrist in Tagen ab Briefdatum (Vorgabe 28)                                                                                                                                                                                                                                                                           | `manage_settings`                           |
| 8   | **Datenschutz & Sitzung**   | Löschfristen (Lesehistorie 90 Tage, Lernmittel-Historie 730 Tage, Anliegen 365 Tage, Audit 24 Monate), Theke leeren nach _n_ Minuten (Vorgabe 5), Sperrbildschirm nach _n_ Minuten (Vorgabe 15)                                                                                                                                                                                                                                                                                                                                                                                                                                     | `manage_settings`                           |
| 9   | **Erreichbarkeit & Alarme** | öffentliche Adresse (Basis für Bestätigungs-Link, Katalog- und Monitor-Adresse; leer = keine Links), Alarm-Empfänger (leer = alle aktiven Admins)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `manage_settings`                           |
| 10  | **Mail**                    | SMTP-Postausgang mit Verbindungstest, Test-Mail; **Mail-Vorlagen** `MAHNUNG_ELTERN` (gedruckter Elternbrief: `{{.Vorname}} {{.Nachname}} {{.BuchListe}} {{.Frist}}`) und `BESTELLUNG_HAENDLER` (Bestellmail: `{{.Datum}} {{.Kundennummer}} {{.AnzahlTitel}} {{.AnzahlExemplare}} {{.BestaetigungsLink}}`) — Platzhalter je Typ, Gate `api/mail_vorlagen_platzhalter_test.go` (Migrationen 023, 052; die tote Vorlage `BESTELLUNG_EINGETROFFEN` ist mit 092 ausgetragen, Abholbereit läuft über den Abholfach-Hinweis am Terminal). Die SMTP-Daten aus der `.env` werden nur beim ersten Start übernommen; danach gilt die Datenbank | `manage_settings`                           |
| 11  | **LMF-Aktionen**            | Massenverlängerung: alle offenen Lernmittel-Ausleihen **einer Klasse** auf ein neues Rückgabedatum (`POST /api/ausleihen/global-extend-lmf`), mit Rückfrage vor dem Ausführen                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `edit_books`                                |
| 12  | **Datenverwaltung**         | Katalog-Import (Littera, CSV/XLSX), Finaler Bestands-Import (Kombi-CSV), Cover-Synchronisation für fehlende Cover, Daten exportieren (Katalog als CSV), **Offline-Sicherungen einspielen** (siehe §18.4)                                                                                                                                                                                                                                                                                                                                                                                                                            | `manage_inventory` / `edit_books`           |
| 13  | **LUSD & Versetzung**       | LUSD-Abgleich (§8) und Klassen-Versetzung zum Schuljahresende; hieß bis 06.09.2026 „Schuljahreswechsel" — derselbe Name wie der LMF-Plan im System-Menü, deshalb umbenannt (Rechte: import_students bzw. manage_students_admin)                                                                                                                                                                                                                                                                                                                                                                                                     | `import_students` / `manage_students_admin` |
| 14  | **Betriebsbereitschaft**    | Selbstprüfung (§15)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `manage_settings`                           |
