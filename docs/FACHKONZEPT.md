...
- **Ferien (seit 24.09.2026):** Eine Frist, die in Tagen zählt — Buch, Medium, Gerät, Verlängerung —, rückt auf den nächsten Schultag, wenn sie auf ein Wochenende, einen gesetzlichen Feiertag Hessens oder in die Ferien fällt (`service.Tagesfrist`, eine Rechnung für alle vier Wege). Die Ferien stehen als Tabelle im Programm: die Sommerferien nach dem KMK-Beschluss samt der Jahre, die die Schule selbst einträgt (`pkg/lmfplan/ferien.go`), Herbst-, Weihnachts- und Osterferien nach den Terminen des Hessischen Kultusministeriums bis 2029/30 (`pkg/lmfplan/schulferien.go`; zwei Jahre vor dem Ende warnt die Betriebsbereitschaft, „Ferien für Leihfristen"). Die beweglichen Ferientage der Schule stehen nicht darin. Nicht verschoben werden Stichtage (Lernmittel, LMF-Plan, Ferien-Leseclub), von Hand gesetzte Fristen und die Jahresfrist der Dauerleihe; der LMF-Stichtag 31.07. liegt in jedem Jahr in den Sommerferien. Eine Ferien-Datei zum Hochladen gibt es nicht — die Termine stehen Jahre im Voraus fest. Das Werkzeug für „Bücher über die Sommerferien mitnehmen" ist der **Ferien-Leseclub** (Kategorie Ausleihe & Fristen, §17): aktiv + Zieldatum → alle Ausleihen bekommen dieses feste Rückgabedatum. Eine Ferien-Pause des Mahnwesens gibt es ebenfalls nicht mehr: Die Tabelle `ferien_schliesszeiten` (Migration 017, Banner + Sperre) hatte nie einen Schreiber und ist mit Migration 102 ausgebaut (entschieden am 06.09.2026: „ausbauen" — das Mahnwesen wird nur von Hand bedient).
- **Lehrer (Handapparat):** Erhalten pauschal eine Frist von einem Jahr — `AddDate(1, 0, 0)`, also ein **Kalenderjahr**, nicht 365 Tage (im Schaltjahr sind es 366). Wie jede andere Frist läuft sie durch `TagesEndeInSchulzeitzone` (`internal/service`, reicht an die eine Definition `schulzeit.TagesEnde` weiter); eine zweite, rohe Berechnung gibt es bewusst nicht.
- **Verlängerungen:** Ausleihen können verlängert werden, es sei denn, der Schüler ist gesperrt oder hat das Ausleihlimit überschritten. Die neue Frist ist die Leihfrist für Bücher (`frist_buch_tage`), gezählt ab dem späteren von alter Frist und heute, und rückt wie jede Tagesfrist auf den nächsten Schultag.
...
- **Sperren: ein Prüfweg für Buch, Gerät und Nachbuchen** (entschieden am 24.09.2026, `pruefeAusleihSperren` in `internal/service/ausleih_sperren.go`). Die Regel richtet sich danach, wer gesperrt hat:
  - **Sperre am Leser** — von Hand (Akte → „Schüler sperren", Begründung Pflicht) oder vom Programm für Ehemalige (Versetzung, LUSD-Import). An der Theke geht dann nur die Rückgabe. Aufgehoben wird sie in der Akte oder im Sperr-Dialog der Theke („Sperre aufheben"), beides mit dem Recht `edit_students` über dieselbe Tür (`PATCH /api/admin/students/{id}/lock`); Aufheben nimmt beide Sperren weg. Wie Littera: „Für eine Ausleihe muss jedoch die Sperre zunächst in den Leserstammdaten aufgehoben werden!"
  - **Hinweise** — eine unbezahlte Forderung, gleich aus welchem Topf (ein Konto je Leser), und die Überfällig-Automatik: ab `MaxOverdueItems` Medien, die länger als `MaxOverdueDays` überfällig sind. Wer Schülerdaten ändern darf, übergeht sie einmalig („Einmalig ignorieren", `override_block`); Littera hält es mit dem Gebührenlimit ebenso.
  - **Lernmittel** (Entscheidung der Schule vom 22.09.2026, Grundlage: Lernmittelfreiheit in Hessen): Beim Schulbuch hält nur die Sperre von Hand auf. Keine automatische Sperre, auch keine übergehbare — auch nicht die der Ehemaligen; ob die Schule das so meint, ist angefragt ([OFFEN.md](OFFEN.md) 8.7).
...
- **Den Sperrgrund** sieht an der Theke nur, wer `view_students` hat; sonst steht dort „bitte an die Bibliotheksleitung wenden" (`ohneSperrgrund` in `api/action.go`).
...
Plans bekommt ihren Termin (`SetzeLernmittelFristFuerKlassenIn`: aktive, nicht gesperrte
...
**Auflagen (seit 25.09.2026, docs/OFFEN.md 4.18, Stufe 5; §13).** Die Ableitung zählt am
Buch, nicht an der Auflage — `COALESCE(werk_id, id)` wie der Bestellbedarf. Je Auflage gezählt
verschwand ein Klassensatz, sobald er auf zwei Auflagen verteilt war (14 Kinder mit der 4., 14
mit der 3.: keine hat mehr als die Hälfte), und bei 20 zu 8 blieb die Mischung unsichtbar. Für
...
- **Sperren des Lesers:** derselbe Prüfweg wie beim Buch (§2.2). Seit dem 24.09.2026 lassen
  sich die Hinweise auch am Gerät übergehen; die Zubehör-Liste reicht das Übergehen weiter, und
  protokolliert wird erst mit der Ausleihe. Bis dahin prüfte das Gerät für sich, ließ nichts
  übergehen und sperrte auch Kollegen.
...
- **Bestellbedarf:** Ein Titel gilt als Bedarf, wenn sein Gesamtbestand unter der Schwelle aus den Systemeinstellungen liegt (`bestellbedarf_schwelle`, Vorgabe 3). Die Spalte `meldebestand` am Titel wird nur noch **informativ** mitgeliefert und löst nichts mehr aus — ihr pauschaler Default 5 meldete früher fast jeden Titel. Die Warnung selbst lässt sich abschalten (`bestellbedarf_warnung_aktiv`). **Gezählt wird am Buch (seit 25.09.2026, docs/OFFEN.md 4.18, Stufe 3):** Sind Auflagen zusammengefasst (§13), steht ihre Summe gegen die Schwelle, und das Buch hat eine Zeile — die der neuesten Auflage, denn die wird bestellt; die Aufschlüsselung je Auflage reist mit (`auflagen`, mit ISBN, damit der Filter der Liste auch die alte Auflage findet). Der Filter nach Topf gilt je Auflage, bevor gezählt wird. Ein Titel ohne Werk ist sein eigenes Buch und wird gezählt wie bisher. Die Bestellliste als PDF liest dieselbe Quelle (`reordersMitSettings`). Belegt: `api/reorders_pg_test.go` (`TestQueryReorders_ZaehltAmBuch`), `BedarfZeile.test.js`, `frontend/e2e/auflagen-bestellbedarf.spec.js`.
...
- **Retention-Blockade:** Ein Abgänger wird **nicht** gelöscht oder anonymisiert, solange er noch Bücher ausgeliehen hat oder unbezahlte Schadensfälle existieren. In diesem Fall wird der Datensatz eingefroren (`ist_gesperrt = true`, Sperrgrund: "Automatisierte Abgänger-Sperre (offene Vorgänge)"). Falls die offenen Vorgänge geklärt werden und der Abgänger im Folgejahr in der LUSD wieder als aktiver Schüler auftaucht, hebt das System die Sperre automatisch wieder auf. Seit dem 24.09.2026 lässt sie sich außerdem in der Akte aufheben (§2.2). Ein späterer LUSD-Import sperrt einen so entsperrten Ehemaligen nicht wieder: Er behandelt nur aktive Schüler als neue Abgänger (`sammleAbgaenger`); die Frist bis zur Anonymisierung läuft unabhängig von der Sperre weiter (`PredikatAnonymisierung`).
...
- Jede administrative Aktion (Benutzer gelöscht, Schadensfall storniert, Leser gesperrt oder entsperrt) wird in der Tabelle `audit_logs` mit `Akteur`, `Zeitstempel`, `IP` und Vorher-/Nachher-Details protokolliert. Sperren und Entsperren stehen dort erst seit dem 24.09.2026 (`LESER_GESPERRT`, `LESER_ENTSPERRT`); dieser Satz nannte sie schon vorher.
...
- **Titel ohne Exemplar stehen in keinem Katalog (seit 22.09.2026, Antwort der Schule auf Punkt 4 der Sichtung vom 16.09.2026):** Die drei Türen der Verwaltung, über die Titel gesucht werden — die Katalogliste `GET /api/books` (Medienkatalog „Suche & Filter", Titel-Verwaltung und die Zuordnung von Klassensätzen), die Theken-Suche (`SearchTitlesFuzzy`) und die Aktionssuche der Omnibox (`SearchTitles`) — zeigen nur Titel mit mindestens einem nicht ausgesonderten Exemplar; der Zulauf zählt als vorhanden. EIN Prädikat, `repository.SQLTitelHatExemplar` (`book_bestand.go`). Der Titel bleibt in der Tabelle, sonst legt ihn jemand ein zweites Mal an: Die Titel-Verwaltung erreicht ihn über die Aufräumsicht `GET /api/books?bestand=ohne` (dasselbe Prädikat mit NOT, Umschalter „Mit Exemplaren | Ohne Exemplare"), die Bestellliste führt ihn unter der Schwelle. Der öffentliche Katalog — über ihn sucht auch _Mein Portal_ (§16.1) — und der Monitor haben eine engere eigene Regel (`OeffentlichSichtbar`: kein Lernmittel, kein Zulauf). Belegt: `repository/titel_bestand_pg_test.go`, `inventur/titel_ohne_exemplar_pg_test.go`.
- **Mehrjahresband (seit 22.09.2026, Antwort der Schule auf Protokoll 5 der Sichtung vom 16.09.2026; Migration 134):** Am Titel steht der Schalter `mehrjahresband` (Titel-Verwaltung, nur bei Lernmitteln). Die Jahreszahl, bis zu der das Buch beim Kind bleibt, ist `jahrgang_bis` aus der Spanne „Im Unterricht von Jahrgang … bis" — eine zweite Zahl gibt es nicht; die Spalte `ziel_jahrgang` (Migration 030, nie beschrieben, am selben Tag kurz mit einer Tür versehen) ist mit 134 gefallen. Die Frist ist der Stichtag des Schuljahres, in dem das Kind `jahrgang_bis` beendet: `AdditionalYears = jahrgang_bis − Jahrgang der Klasse`, dann `AddDate` am Stichtag (`internal/service/loan_rules.go`). Vor dem Rückgabetermin der Klasse geht der Termin das Buch nichts an; ist er vorbei, rechnet die Frist vom folgenden Schuljahr aus, und eines der Jahre steckt in diesem Sprung. Der Schalter gilt nur an einem Lernmittel und nur mit einer Spanne über mehr als einen Jahrgang — an beiden Türen der Titel-Verwaltung (`inventur/mehrjahresband.go`, 400) und in der Datenbank (`chk_mehrjahresband_spanne`) dieselbe Regel. Das Schuljahr, das Littera getrennt führt, steckt hier im Datum der Frist. Belegt: `internal/service/lmf_frist_termintag_pg_test.go`, `inventur/mehrjahresband_am_titel_pg_test.go`.
- **Auflagen eines Schulbuchs (Migration 148, docs/OFFEN.md 4.18; seit 25.09.2026):** Eine neue Auflage ist ein eigener Titel mit eigener ISBN — sie hat andere Seitenzahlen, und jedes Exemplar bleibt an seiner Auflage. Auflagen desselben Buchs lassen sich zusammenfassen: Sie tragen dieselbe `werk_id` (Tabelle `werke`, ohne Namen — jede Ansicht zeigt die neueste Auflage mit ihrem Titel). Gelesen wird über `COALESCE(werk_id, id)`; ein Titel ohne Werk ist sein eigenes. Geschrieben wird nur in `repository/auflagen.go`, nicht beim Speichern der Titelmaske (`UpdateBook`), und dort stehen die Regeln: nur Lernmittel (400 mit Text, der den Titel nennt); gehören beide Titel schon zu verschiedenen Büchern, werden alle zu einem; ein Werk mit weniger als zwei Titeln fällt samt dem letzten Verweis — auch, wenn ein Titel gelöscht wird: Beide Löschwege (`DELETE /api/buecher/titel/{id}`, `repository.DeleteTitle`; `DELETE /api/books`, `inventur.DeleteBooks`) nehmen als Erstes die Sperre der Auflagen (`repository.WerkeDerTitel`) und räumen nach dem Löschen auf (`RaeumeWerkeAuf`). „Die neueste Auflage" ist das jüngste Erscheinungsjahr, bei gleichem oder fehlendem Jahr die zuletzt angelegte (`repository.SQLNeuesteAuflageZuerst`). Türen: `GET /api/buecher/titel/{id}/auflagen` (`view_books`, die Auflagen mit Bestand in den Zahlen des Katalogs), `POST` und `DELETE` desselben Pfads (`edit_books`, Zusammenfassen und Lösen). **In der Titelmaske** steht der Abschnitt _Auflagen_ (`BuchAuflagen.svelte`) bei jedem Lernmittel und bei jedem Titel, der schon zu einem Buch gehört — auch wenn er inzwischen kein Lernmittel mehr ist, sonst ließe er sich nicht lösen. Er zeigt die Auflagen, die neueste zuerst, mit Bestand und der Summe; Lösen fragt nicht nach, weil es sich jederzeit zurücknehmen lässt und nichts löscht. Der Dialog _Andere Auflage zuordnen_ sucht über die Tür der Titel-Verwaltung zweimal — `GET /api/books?q=` mit und mit `bestand=ohne` —, weil eine alte Auflage oft kein Exemplar mehr hat, und bietet nur Lernmittel an, die noch nicht zu diesem Buch gehören (`auflagenSuche.svelte.js`: nur die jüngste Antwort zählt, ein Fehler heißt Fehler). Gehört der gewählte Titel schon zu weiteren Auflagen, sagt der Dialog vor dem Klick, dass sie mitkommen; scheitert diese Nachfrage, sagt er das statt „keine". Im Bestellbedarf zählt das Buch (§7). **Neue Auflage bestellen** (Menü der Zeile im Bestellbedarf, `NeueAuflageDialog.svelte`): Die ISBN der neuen Auflage holt den Titel über die Tür der Titelsuche (`POST /api/buecher/aus-isbn`, aus dem Katalog oder neu aus der DNB), der Dialog zeigt ihn als Vorschlag, und erst die Bestätigung führt ihn als Lernmittel (`PUT …/lernmittel`), fasst ihn mit der Zeile zusammen (`POST /api/buecher/titel/{id}/neue-auflage`, `create_orders` — derselbe Handler wie die Titelmaske; „Der Besteller legt das Werk an", entschieden am 23.09.2026) und legt ihn in den Warenkorb. Ist der gefundene Titel schon Teil des Buchs, bietet der Dialog nichts zum Zuordnen an. **An der Theke** (Stufe 5): Bekommt ein Kind ein Lernmittel in einer anderen Auflage, als Kinder derselben Klasse sie gerade haben, trägt die Antwort des Scans `auflagen_hinweis` (Klasse, diese Auflage, die anderen mit der Zahl der Kinder, keine Namen; `repository.AuflagenMischungInKlasse`, „dieselbe Klasse" über `klassen_normkey` wie in der Klassensatz-Übersicht, nur offene Ausleihen aktiver Kinder). Gefragt wird nach dem Commit der Ausleihe; ein Fehler dabei landet im Log und hält die Buchung nicht auf. Die Theke blitzt orange und zeigt die Zeile über dem Konto bis zum nächsten Scan (entschieden am 25.09.2026: wie die Fremdrückgabe, kein Dialog). **In der Klassensatz-Übersicht** (Stufe 5, §4.3) zählt das Buch: Ein Satz aus den Ausleihen entsteht aus den Kindern mit irgendeiner Auflage, und die Kachel trägt die Auflagen der Klasse mit der Zahl der Kinder, sobald Kinder eine andere Auflage als die der Kachel haben. **Im Medienkatalog** (Stufe 6, _Suche & Filter_) ist ein Buch eine Kachel: Die Katalogliste `GET /api/books` trägt je Titel `werkId` und `werkRang` (1 = die neueste Auflage nach `SQLNeuesteAuflageZuerst`, gezählt über alle Auflagen des Buchs), und erst die Anzeige fasst nach der Suche zusammen (`buecherJeBuch` in `startseiten_api.js`) — die Titel-Verwaltung und der Zuordnen-Dialog lesen dieselbe Liste und brauchen jede Auflage einzeln. Oben steht die getroffene Auflage (nach einer gescannten ISBN genau diese, sonst die neueste), der Bestand ist die Summe aller Auflagen aus der ganzen Liste, darunter die Aufschlüsselung wie im Bestellbedarf. **Im Portal** (Reiter „Schulbücher", Stufe 6) rechnet der Server so: Die Filter (Jahrgang, Zweig, Suchtext) wählen die Bücher — trifft eine Auflage, gehört das Buch dazu —, gezählt wird über alle Schulbuch-Auflagen des Buchs, oben steht die neueste getroffene, und die Zeile trägt `auflagenBestand` (`lernmittelJeBuchSQL` in `inventur/datenbank_lernmittel.go`). Liste, Fach-Zahlen und PDF lesen dieselben Zeilen; die Fach-Zahlen zählen Bücher statt Auflagen, und das PDF setzt die Aufschlüsselung unter den Titel (`auflagenBeschriftung` in `lernmittel_pdf.go` ist der Zwilling der Browser-Regel, beide lesen dieselben Prüffälle aus `auflagenText.faelle.json`). **Dieselbe ISBN in der anderen Länge** (Stufe 4): Findet die Tür der Titelsuche (`POST /api/buecher/aus-isbn`) die ISBN nicht in dieser Schreibweise, wohl aber als ISBN-10 oder ISBN-13 mit 978 (`isbnutil.AndereForm`, der Zwilling von `isbnFormen.js`), legt sie nichts an und antwortet mit `andere_form` ohne `titel_id`. Die Titelsuche und „Neue Auflage bestellen" fragen dann „Ist es dasselbe Buch?" mit zwei Zeilen, _Diesen Titel nehmen_ und _Neu anlegen_ (`AndereIsbnFormWahl.svelte`; kein Rückfrage-Dialog, dort zählte Escape als „nein" und legte still einen zweiten Titel an); erst _Neu anlegen_ fragt mit `neu_anlegen` die DNB. Die Frage gilt der ISBN der Antwort, nicht dem Suchfeld. **Keine Vorschlagsliste zum Zusammenfassen:** Gemessen am Testserver am 25.09.2026 (lesend) standen nur zwei Lernmittel mit gleichem Titel und Verlag im Katalog, und höchstens eines der beiden hatte Exemplare; zusammengefasst wird deshalb beim Nachbestellen und von Hand in der Titelmaske. Die Zählung sieht nur gleiche Titel — eine Neuauflage unter geändertem Titel („… Neubearbeitung") nicht. Vorgeschlagen statt übernommen, weil die Rechnung von einer ISBN-10 mit falschem Prüfzeichen auf die ISBN-13 eines anderen Buchs führt (am Testserver `3499500252`, docs/OFFEN.md 5.5). Belegt: `repository/auflagen_pg_test.go`, `api/auflagen_pg_test.go`, `BuchAuflagen.test.js`, `AuflageZuordnenDialog.test.js`, `auflagenSuche.svelte.test.js`, `frontend/e2e/auflagen-zusammenfassen.spec.js`, `NeueAuflageDialog.test.js`, `frontend/e2e/auflagen-neue-auflage.spec.js`, `repository/auflagen_klasse_pg_test.go`, `api/auflagen_theke_pg_test.go`, `omniboxAuflagenHinweis.test.js`, `frontend/e2e/auflagen-theke.spec.js`, `inventur/klassensatz_auflagen_pg_test.go`, `KlassenBuchKachel.test.js`, `frontend/e2e/auflagen-klassensatz.spec.js`, `inventur/katalog_auflagen_pg_test.go`, `startseiten_api.test.js`, `BuchKarte.test.js`, `frontend/e2e/auflagen-katalog.spec.js`, `inventur/lernmittel_pg_test.go`, `inventur/lernmittel_pdf_test.go`, `frontend/e2e/auflagen-portal.spec.js`, `repository/auflagen_loeschen_pg_test.go`, `inventur/auflagen_loeschen_pg_test.go`, `pkg/isbnutil/andere_form_test.go`, `api/isbn_andere_form_pg_test.go`, `OrderSearch.test.js`, `frontend/e2e/auflagen-isbn-andere-form.spec.js`.
...
- **Schlagworte (Migrationen 138, 143, 144; docs/OFFEN.md 4.20):** Frei eintragbar wie in Littera („Schlagworte (Wertehilfe)"), mehrere je Titel, mit Vorschlägen aus dem Bestand (die 500 häufigsten, `GET /api/schlagworte`). Eingetragen wird im Buchformular und im Bestellkorb; beide schreiben über EINEN Pfad, `repository.SetzeSchlagworte`: Leerraum wird zusammengezogen, Doppelte fallen, höchstens 30 Wörter je Titel und 80 Zeichen je Wort, und eine vorhandene Schreibweise gewinnt — die Identität eines Wortes ist seine Kleinschreibung (eindeutiger Index auf `lower(wort)`). Zusammengehalten wird die Liste durch die Pflege unter _Einstellungen → Schlagworte_ (Recht `edit_books`, wie die Systematik): Umbenennen und Zusammenführen ändern alle Titel auf einmal und lassen die alte Schreibweise wahlweise als **Verweis** stehen (Pflichtfeld `alte_als_verweis`, das Kästchen steht auf „behalten"). Ein Verweis leitet eine Schreibweise auf ein Wort („Tierfantasy" → „Fantasy"); wer ihn am Titel einträgt, bekommt das Ziel. Löschen nimmt ein Wort aus allen Titeln, Verweise darauf fallen mit — einzeln oder mehrere auf einmal, alle oder keins (`POST /api/schlagworte/loeschen`; Littera: Dienstprogramme → Datenbearbeitung). Die Regeln der Verweise hält auch die Datenbank (Trigger aus 143, seit 144 mit Sperre gegen gleichzeitige Schreiber): kein Verweis auf sich selbst, keine Kette, kein Titel an einem Verweis, kein Verweis als Filter. **Suche (seit 23.09.2026):** Der Medienkatalog (beide Reiter), der öffentliche Katalog und _Mein Portal_ finden einen Titel auch über seine Schlagworte und über die Verweise darauf — wer „Tierfantasy" sucht, findet, was „Fantasy" trägt. Verglichen wird wie bei Titel und Autor: Teilstring, ohne Rücksicht auf Groß- und Kleinschreibung. Die Regel steht einmal in `repository/schlagworte_suche.go` und löst ein Wort auf wie der Schreibpfad (`coalesce(verweis_auf, id)`) — als Bedingung am Server (Titel-Verwaltung `GET /api/books?q=`, `GET /api/public/opac/suche`) und als Wörter je Titel in der Katalogliste (`suchwoerter`), weil „Suche & Filter" die ganze Liste lädt und im Browser sucht. **Filter im Portal (seit 23.09.2026):** Die markierten Wörter stehen in _Mein Portal → Suchen & Reservieren_ als Filter-Chips unter der Suche (`ui/FilterChips.svelte`, einfach wählbar, ein zweiter Klick nimmt den Filter zurück). Die Liste kommt aus `GET /api/public/opac/filter` und nennt nur Wörter, zu denen der öffentliche Katalog einen Titel zeigt; ein Filter, der nichts fände, erscheint nicht. Gefiltert wird über `schlagwort_id` an der OPAC-Suche (§16.1): ohne Suchtext alle Titel des Worts, mit Suchtext die, die beides treffen. **DNB-Vorschlag beim Bestellen per ISBN (seit 23.09.2026):** Entsteht ein Titel beim Bestellen neu aus der DNB (`POST /api/buecher/aus-isbn`), gleicht der Server die Gattungsbegriffe (655 $a) und die freien Verlagswörter (653 $a ohne Vorsatz — „(Zielgruppe)", „(VLB-WN)" und Ähnliches sind Angaben anderer Art) gegen die eigene Liste ab (`repository.SchlagworteAusStichwoertern`). Vorgeschlagen wird ein Wort, das mindestens ein Titel trägt, direkt oder über einen Verweis — das ganze Wort, nicht ein Teil davon, sonst schlüge „Kriegsende" „Krieg" vor. Das Bestellfenster bietet die Wörter zum Anklicken an (`ui/ChipFeld`, `angebote`); eingetragen wird nur, was jemand übernimmt, denn ein Satz nennt etwa „Deutsch" für die Sprache. Nicht gebaut: ein Vorschlag für Titel, die es schon gibt, und im Buchformular. Noch nicht gebaut: Der Littera-Import leitet aus MAB 710 nur das Fach ab, statt die Wörter zu übernehmen. Belegt: `repository/schlagworte_pg_test.go`, `repository/schlagworte_pflege_pg_test.go`, `repository/schlagworte_loeschen_pg_test.go`, `repository/schlagworte_suche_pg_test.go` (Server und Browser finden dieselben Titel), `inventur/schlagwort_suche_pg_test.go`, `api/opac_schlagwort_pg_test.go` (auch Filterliste und gefilterte Suche), `frontend/src/lib/components/portal/portalSuche.svelte.test.js`, `frontend/e2e/portal-schlagwort-filter.spec.js`, `inventur/dnb_stichwoerter_test.go`, `repository/schlagworte_vorschlag_pg_test.go`, `api/isbn_schlagwort_vorschlag_pg_test.go`, `frontend/src/lib/components/bestellungen/OrderSearch.test.js`, `frontend/e2e/schlagworte-roundtrip.spec.js`, `frontend/e2e/schlagworte-pflege.spec.js`, `frontend/e2e/schlagworte-mehrere-loeschen.spec.js`.
...
**Geprüft werden zwanzig Bereiche**, darunter: Auslagerung der Backups, Geheimnisse, Schlüssel und
...
Für Schülerinnen, Schüler, Eltern und Kollegium: Suche nach Titel, Autor, ISBN oder Schlagwort
(Volltext über `search_vector` **oder** Teilstring; ein Verweis findet sein Schlagwort, §13;
`api/opac.go`), Trefferkarte mit Cover und
