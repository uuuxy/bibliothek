...
| `IMAP_PORT`                                                                | IMAP-Port                                                                                                                                                                                                                                    | Standard: 993. Wirkt im Docker-Betrieb nicht, solange `docker-compose.yml` sie nicht durchreicht (offen, siehe OFFEN.md 4.7)                                                                                                               |
...
| `ALLOWED_ORIGIN`                                                           | Erlaubte Herkunft für CORS (die Frontend-Adresse der Schule)                                                                                                                                                                                 | Empfohlen in Produktion. Wirkt im Docker-Betrieb nicht, solange `docker-compose.yml` sie nicht durchreicht (offen, siehe OFFEN.md 4.7)                                                                                                     |
...
| `SMTP_ALLOW_INSECURE_TLS`                                                  | TLS-Zertifikatsprüfung deaktivieren (TLS bleibt)                                                                                                                                                                                             | Nur für Legacy-SMTP-Server. Wirkt im Docker-Betrieb nicht, solange `docker-compose.yml` sie nicht durchreicht (offen, siehe OFFEN.md 4.7)                                                                                                  |
| `SMTP_ALLOW_PLAINTEXT`                                                     | Versand **ganz ohne** TLS erlauben, wenn der Server kein STARTTLS anbietet                                                                                                                                                                   | Nur für ein Legacy-Relay. Ohne diese Variable bricht der Versand in dem Fall ab, statt Mahntexte im Klartext zu schicken. Wirkt im Docker-Betrieb nicht, solange `docker-compose.yml` sie nicht durchreicht (offen, siehe OFFEN.md 4.7)    |
...
| `RATE_LIMIT`                                                               | Globales Limit in Requests/s/IP                                                                                                                                                                                                              | Vorgabe **50** (der Produktionswert). Ein unlesbarer Wert wird protokolliert und ignoriert, es gilt dann die Vorgabe. Wirkt im Docker-Betrieb nicht, solange `docker-compose.yml` sie nicht durchreicht (offen, siehe OFFEN.md 4.7)        |
...
| `SENTRY_DSN`                                                               | Sentry Error Tracking                                                                                                                                                                                                                        | Optional. Wirkt im Docker-Betrieb nicht, solange `docker-compose.yml` sie nicht durchreicht (offen, siehe OFFEN.md 4.7)                                                                                                                    |
...
> und einer mit `-hex 16`. Der liefert 32 Zeichen, die der Server als Schlüssel annimmt
> (er akzeptiert 32 oder 64 Zeichen) — aber nur 128 statt 256 Bit Zufall.
...
2. Dateinamen in `schema.sql` unter `schema_migrations` eintragen (geprüft von `TestMigrationSeedListMatchesFiles` in `db/migrations_drift_test.go`)
...
Reihenfolge auf dem Server (Variablen an die eigene `.env` anpassen, keine Platzhalter in
...
### 7.0 Externes Uptime-Signal (5-Minuten-Handgriff, offen: OFFEN.md 7.5)
...
healthcheck:
  test:
    [
      "CMD-SHELL",
      "pg_isready -U ${POSTGRES_USER:-postgres} -d ${POSTGRES_DB:-bibliothek}",
    ]
```
...
`./update.sh` fragt beim Deploy **beide Quellen** ab (Docker-Status _und_ `/health` im
