...
> Zuletzt aktualisiert: 2026-09-26 (Protokoll: die einmalige Bereinigung zu gelöschten Lesern,
> Migration 147; übergangen wird ein Hinweis, keine Sperre).
> Davor 2026-09-25 (Ausnahmeliste von govulncheck leer: GO-2026-6452 gilt in
...
- Alle schützenswerten Endpunkte sind über `RequirePermission` abgesichert (im Inventur-Modul über die hereingereichten Wächter `RequireViewBooks`, `RequireEditBooks`, `RequireDeleteBooks` und `RequireAuthenticated`); `RequireRoles` ist am 04.08.2026 entfallen.
...
- **PII-Matrix (seit 18.08.2026):** [`docs/PII_MATRIX.de.md`](PII_MATRIX.de.md) stuft jede Route nach Schülerdaten ein (Stufe 0–3) und nennt das Recht davor; `api/pii_matrix_test.go` hält Tabelle und Registrierung deckungsgleich — eine neue Route ohne Einstufung wird rot. Der Sperrgrund-Freitext (`block_reason`) ist Stufe 2 und wird ohne `view_students` überall ausgeblendet (`ohneSperrgrund`). Seit dem 01.09.2026 ist die Stufe **gemessen, nicht nur behauptet**: `api/pii_antwort_gate_pg_test.go` ruft jede GET-Route und die lesenden POSTs (Theken-Scan samt Batch, LUSD-Vorschau) über den echten Router mit genau dem Recht ihrer Zeile auf und prüft die Antwort gegen Kanarienwerte je Stufe.
...
| Fremdbeförderung     | `POST /api/benutzer`/`PUT /api/benutzer/{id}` mit `rolle:"admin"` für ein beliebiges Konto                                                                                                                                                                                                | Die Rolle `admin` vergibt nur ein Administrator                                                |
...
| `GET /api/public/opac/filter`                                                                 | Die Schlagworte, die im Portal als Filter stehen (Kennung und Wort) — Katalogdaten, nur Wörter mit einem öffentlich sichtbaren Titel.                                                                                                   |
...
- **Lesehistorie befristen (`RunLesehistorieBefristung`, seit 22.08.2026):** Trennt abgeschlossene Ausleihen nach Frist vom Schüler (`schueler_id = NULL`); der Vorgang bleibt für Statistik und Bestandskartei erhalten, nur ohne Person. Zwei Fristen, weil zwei Verarbeitungstätigkeiten (Einstellungen → „Datenschutz & Sitzung", 0 = aus): **Schülerbücherei 90 Tage** (HBDI-Muster-VVT: löschen, „sobald nicht mehr notwendig"), **Lernmittel 730 Tage** (Bestandskartei weist Ausleihe **und** Rücklauf nach, HKM-Leitfaden LMF 11.3; Schadensersatz läuft über die Schulaufsicht, 12.3–12.7). Ausleihen mit **offenem Schadensfall** bleiben zugeordnet. Seit dem 16.09.2026 gilt die Befristung **jedem Leser**, auch dem Kollegium (bis Migration 125 traf sie von selbst nur Schüler, weil eine Lehrerausleihe in einer anderen Spalte stand); eine laufende Dauerleihe ist nicht betroffen, weil die Frist mit der Rückgabe beginnt. Gate: `jobs/cron_dsgvo_lesehistorie_pg_test.go` (Paarung Frist × Medienklasse, Schadensfall-Wächter, Aus-Schalter — am Rückbau rot gesehen). Vorher behielt jede Ausleihe ihre `schueler_id` bis zur Schüler-Löschung; die Titel-Historie zeigte bis zu 200 Entleiher mit Namen.
- **Anonymisierung alter Datensätze (`RunGDPRAnonymizeOldData`):** Leert nach Frist (Soft-Delete > 180 Tage; Abgänger nach der einstellbaren **Karenzzeit** `abgaenger_karenz_tage`, Vorgabe 90 Tage ab dem spätesten von Abgang `abgaenger_seit` und letztem abgeschlossenem Vorgang `letzter_vorgang_am` — letzte Rückgabe oder letzter Schadensabschluss, seit Migration 137 als Spalte am Leser gespeichert statt zur Laufzeit über `ausleihen.schueler_id` gerechnet, weil die Lesehistorie-Befristung genau diese Zuordnung löst und die Karenz sonst verkürzte; der Zeitpunkt steht in der Art.-15-Auskunft und sagt nichts darüber, was ausgeliehen war — bis 02.09.2026 fest 360 Tage, bis 05.09.2026 nur ab dem Abgang) alle direkt identifizierenden Spalten eines Schülers (Name, Adresse, Geburtsdatum, Schuleintritt, LUSD-ID, Eltern-E-Mail), ersetzt die Ausweisnummer durch `ANON-…`, löscht das verschlüsselte Foto — und tilgt die PII-Spuren aus den **Neben-Tabellen**: den Klarnamen aus `audit_log`, die LUSD-ID, die Ausweisnummern und den Sperrgrund aus `audit_logs`, und die Vormerkungen des Schülers. Ohne diese Tilgung überlebte der Personenbezug bis zur Audit-Aufbewahrung (bis zu 24 Monate) nach der eigentlichen Löschung. Einträge in `audit_logs` zu Lesern, die schon gelöscht waren, bevor der Sperrgrund zur Tilgung gehörte, erreicht der nächtliche Lauf nicht; sie hat Migration 147 einmal nach derselben Anweisung bereinigt (25.09.2026). Eine `A-`-Nummer, die dabei — oder beim endgültigen Löschen, Zusammenführen, Leeren, Umschreiben — aus `leser` verschwindet, hält ein Trigger in `ausweisnummern_ausgeschieden` fest: nur die Zahl, ohne Person und ohne Datum. Der Generator vergibt sie deshalb nicht noch einmal, und eine alte Karte findet an der Theke niemanden (Migration 146).
