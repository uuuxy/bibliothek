...
| Phantom-Erfolg                | Schreibpfad ohne `RowsAffected`-Prüfung: 0 Zeilen = „success" (+ Audit-Eintrag über eine Nicht-Änderung) | PG-Tests je Pfad (`api/copy_delete_pg_test.go`, `api/aussondern_tueren_pg_test.go`)                    | 31.08.2026: DeleteCopy, DecommissionCopy, UpdateCopyStatus gefixt — **kein Gate über den Bestand**, Prüfvorrat |
