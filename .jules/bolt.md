...
## 2026-08-10 - [Optimize copy admin n+1]
**Learning:** Found a performance bottleneck in GetTitleCopiesHandler (api/copy_admin.go) where a scalar subquery SELECT COUNT(*) = 0 was used in the SELECT list. PostgreSQL evaluates this for every row, resulting in an N+1 query pattern.
**Action:** Always prefer NOT EXISTS(SELECT 1 ...) over SELECT COUNT(*) = 0 for simple existence checks in PostgreSQL, as it short-circuits evaluation upon the first match instead of scanning the entire result set.
