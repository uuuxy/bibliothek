...
## 2026-06-29 - [Refactor N+1 Query in ProcessOrder with CopyFromRows]
**Learning:** Replaced individual INSERT queries inside a loop with a bulk CopyFrom operation in `api/order_service.go`. Using `tx.CopyFrom` together with `pgx.CopyFromRows` effectively removes N+1 bottlenecks when creating multiple row inserts simultaneously.
**Action:** Use `CopyFrom` via `pgx` for bulk data insertions (like creating order line items) to drastically improve database performance.
