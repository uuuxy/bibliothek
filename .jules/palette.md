...
## 2024-05-27 - Added aria-labels to quantity buttons
**Learning:** Found that custom increment/decrement quantity buttons in `OrderCart.svelte` lacked `aria-label` attributes for screen readers, causing them to only be announced as "+" and "-".
**Action:** Added explicit `aria-label`s ("Menge verringern" and "Menge erhöhen") to these buttons to improve screen reader accessibility.
