...
test('Ausleihlimit 1: zweiter Checkout blockt sofort', async ({ page }) => {
	...
	const vorher = querySQL(
		`SELECT wert FROM system_einstellungen WHERE schluessel = 'max_ausleihen_schueler'`
	);
	...
	try {
		...
		if (vorher) {
			seedSQL(
				`UPDATE system_einstellungen SET wert = '${vorher}' WHERE schluessel = 'max_ausleihen_schueler';`
			);
		} else {
