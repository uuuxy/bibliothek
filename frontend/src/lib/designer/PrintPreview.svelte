<script>
  ...
  const frontEls = $derived(
    idStore.front.elements.filter(e => e.show).slice().sort((a, b) => a.zIndex - b.zIndex)
  );
  /** Elements visible on the back side, sorted by zIndex ascending. */
  const backEls = $derived(
    idStore.back.elements.filter(e => e.show).slice().sort((a, b) => a.zIndex - b.zIndex)
  );
</script>
...
<style>
  @media print {
    :global(html, body) { margin: 0 !important; padding: 0 !important; background: white !important; overflow: hidden !important; }
    :global(main, .min-h-screen, .flex) { margin: 0 !important; padding: 0 !important; display: block !important; background: white !important; border: none !important; box-shadow: none !important; }
    :global(.no-print) { display: none !important; }
    :global(body[data-print-mode="card"]) .print-section-card { display: block !important; }
    :global(body[data-print-mode="card"][data-print-side="back"]) .print-section-card { display: none !important; }
    :global(body[data-print-mode="a4"][data-print-side="back"]) .print-section-a4 { display: none !important; }
    :global(body[data-print-mode="card"][data-print-side="back"]) .print-section-back-card { display: block !important; }
    :global(body[data-print-mode="a4"][data-print-side="back"]) .print-section-back-a4 { display: block !important; }
  }
</style>
