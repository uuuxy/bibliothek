...
<div class="w-full space-y-4 no-print">
  ...
  <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
    {@render toggleGroup(
      [{ value: 'card', label: 'Kartendrucker' }, { value: 'a4', label: 'A4-Bogen' }],
      printMode,
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 bg-slate-50 border border-slate-100 rounded-2xl p-4">
    <div class="space-y-1">
      ...
      {:else}
        <div class="text-xs text-slate-400 font-medium py-2">{loadingStudents ? 'Lade…' : 'Keine Klassen'}</div>
      {/if}
    </div>
    ...
    <div class="space-y-1">
      ...
      <select
        value={barcodeType}
        onchange={(e) => onBarcodeType(/** @type {'code39'|'qr'} */ (/** @type {HTMLSelectElement} */ (e.currentTarget).value))}
        class="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs focus:outline-none"
      >
  ...
  <div class="flex flex-wrap items-center gap-3">
    ...
    <!-- Multi-image upload -->
    <label class="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-xs font-bold text-slate-700 transition-colors cursor-pointer">
      + Bild(er)
      <input type="file" accept="image/*" multiple class="sr-only" onchange={handleMultiImageUpload} />
    </label>

    {#if previewStudent}
      <span class="text-xs text-slate-500 font-medium ml-auto">
        Vorschau: {previewStudent.vorname} {previewStudent.nachname} (Klasse {previewStudent.klasse})
      </span>
    {/if}
  </div>
</div>
  ...
  <div class="flex bg-slate-100 p-0.5 rounded-xl border border-slate-200/40 text-xs shrink-0">
    {#each options as opt}
      <button
        ...
        class="px-3 py-1.5 rounded-lg font-bold transition-all cursor-pointer {active === opt.value ? 'bg-white text-slate-800 shadow-xs' : 'text-slate-500 hover:text-slate-700'}"
      >{opt.label}</button>
    {/each}
  </div>
{/snippet}
