...
{#if status}
	{#if collapsed}
		...
		{#if status.status !== 'ok'}
			<div class="flex justify-center py-2" title="⚠️ {message}">
				<span
					class="w-2.5 h-2.5 rounded-full animate-pulse {status.status === 'critical'
						? 'bg-rose-500'
						: 'bg-amber-500'}"
				></span>
			</div>
	...
	{:else}
		<div
			class="mx-3 my-2 px-3 py-2.5 rounded-xl border text-xs font-bold flex items-start gap-2
