...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
  <div class="space-y-1.5">
    ...
    <select id="supplier" bind:value={orderStore.selectedSupplierId} class="w-full px-3 py-2.5 rounded-lg border border-slate-200 text-base bg-white">
      {#each orderStore.suppliers as s}<option value={s.id}>{s.name} ({s.customerNumber})</option>{/each}
    </select>
  </div>
  <div class="space-y-1.5 relative">
    <label for="book" class="block text-sm font-medium text-slate-600">Buchtitel hinzufügen</label>
    <input id="book" type="text" bind:value={orderStore.searchQuery} oninput={() => orderStore.handleSearchInput()} placeholder="Titel, Autor oder ISBN suchen..." class="w-full px-3 py-2.5 rounded-lg border border-slate-200 text-base bg-white" />
  ...
  <div class="p-4 border-l-2 border-blue-400 bg-blue-50/50 flex flex-col md:flex-row items-center justify-between gap-4 animate-fade-in">
    ...
    <div class="flex flex-wrap items-center gap-4 shrink-0">
      ...
      <div class="flex items-center gap-2 ml-auto">
        <button onclick={() => stagedBook = null} class="px-3 py-1.5 text-sm font-bold text-slate-500 hover:text-slate-700 cursor-pointer">Abbrechen</button>
