...
<div class="space-y-6">
  <div class="flex items-center justify-between border-b border-slate-200 pb-4">
    <div>
      ...
      <p class="text-sm text-slate-500 mt-0.5">Alle aufgegebenen Bestellungen — automatisch erfasst beim Bestellen</p>
    </div>
    {#if bestellungen.length > 0}
      <div class="text-right">
        <div class="text-xs text-slate-400 uppercase tracking-wide font-semibold">Gesamtausgaben</div>
  ...
  {#if loading}
    <div class="py-16 text-center text-slate-400 text-base animate-pulse">Lade Bestellhistorie…</div>
        ...
        <div class="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-xs">
          ...
          >
            <div class="flex items-center gap-6 flex-1 min-w-0">
              <div class="shrink-0">
                <div class="text-xs text-slate-400 font-semibold uppercase tracking-wide">Datum</div>
                <div class="font-bold text-slate-800 text-sm">{datum(b.bestelldatum)}</div>
              </div>
              <div class="min-w-0">
                <div class="text-xs text-slate-400 font-semibold uppercase tracking-wide">Lieferant</div>
                <div class="font-bold text-slate-800 text-sm truncate">{b.lieferant_name}</div>
                <div class="text-xs text-slate-400 truncate">{b.kundennummer ? "Kd.-Nr. " + b.kundennummer : b.lieferant_email}</div>
              </div>
              <div class="shrink-0">
                <div class="text-xs text-slate-400 font-semibold uppercase tracking-wide">Exemplare</div>
            ...
            <div class="text-slate-400 shrink-0 text-lg transition-transform {expandedId === b.id ? 'rotate-180' : ''}">▾</div>
          </button>
            ...
            <div class="border-t border-slate-100 bg-slate-50/40 px-5 py-4">
              ...
              {:else}
                <table class="w-full text-sm border-collapse">
                  <thead>
                    <tr class="text-xs font-semibold text-slate-400 uppercase tracking-wide border-b border-slate-200">
                  ...
                  <tbody class="divide-y divide-slate-100">
                    {#each b.positionen as p}
                      <tr>
                        ...
                        <td class="py-2 text-right font-bold text-slate-800">{euro(p.gesamtpreis)}</td>
                      </tr>
                    {/each}
                  </tbody>
                  <tfoot>
                    <tr class="border-t-2 border-slate-200">
                      <td colspan="4" class="pt-2 text-right font-bold text-slate-600 text-sm">Summe</td>
                      <td class="pt-2 text-right font-black text-slate-900">{euro(b.gesamtbetrag)}</td>
                    </tr>
                  </tfoot>
                </table>
              {/if}
