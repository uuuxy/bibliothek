<script>
  ...
  let {
    incomingShipments = [],
    totalItems = 0,
    selectedExemplarIds = $bindable([])
  } = $props();

  let allSelected = $derived(
    selectedExemplarIds.length > 0 && 
    selectedExemplarIds.length === incomingShipments.flatMap((/** @type {any} */ s) => s.items.flatMap((/** @type {any} */ i) => i.exemplar_ids || [])).length
  );
  ...
  function toggleAll() {
    if (allSelected) {
      ...
      selectedExemplarIds = incomingShipments.flatMap(s => s.items.flatMap((/** @type {any} */ i) => i.exemplar_ids || []));
    }
  }
          ...
          <table class="w-full text-left text-sm border-collapse">
            <tbody class="divide-y divide-slate-100 bg-white">
              {#each group.items as item}
                {@const isSelected = item.exemplar_ids.every((/** @type {string} */ id) => selectedExemplarIds.includes(id))}
