...
<div class="space-y-3">
  ...
  {:else}
    <div class="border border-slate-100 rounded-lg overflow-hidden divide-y divide-slate-100">
      {#each orderStore.cart as item, idx}
        <div class="p-3 bg-slate-50/30 flex items-center justify-between gap-4 text-base">
          ...
          <div class="flex items-center gap-4">
            ...
            <button onclick={() => orderStore.removeFromCart(idx)} class="text-slate-400 hover:text-rose-500 cursor-pointer">Löschen</button>
          </div>
        </div>
      {/each}
    </div>
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-4">
      ...
      <div class="flex flex-col sm:flex-row items-end sm:items-center gap-4">
        <label class="flex items-center gap-2 cursor-pointer bg-white px-3 py-2 border border-slate-200 rounded-lg">
          <input type="checkbox" bind:checked={orderStore.attachBarcodes} class="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500" />
          <span class="text-sm font-bold text-slate-700">Barcodes mitschicken</span>
        </label>
        <button onclick={() => orderStore.submitOrder()} disabled={orderStore.submitting || !orderStore.selectedSupplier} class="px-5 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-bold text-base cursor-pointer disabled:bg-slate-200 disabled:text-slate-400 flex items-center gap-2">
