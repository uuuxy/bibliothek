    ...
    <div class="shrink-0">
      {#if confirmingId === r.id}
        <div class="flex items-center gap-2">
          <button onclick={cancelConfirm} disabled={completingId === r.id} class="px-3 py-2 rounded-full bg-slate-100 hover:bg-slate-200 disabled:opacity-50 text-slate-650 text-xs font-bold transition-colors cursor-pointer">
            Abbrechen
          </button>
          <button
            ...
            {#if completingId === r.id}
              <span class="w-3 h-3 border-2 border-white/60 border-t-white rounded-full animate-spin"></span>
            {:else}
      ...
      {:else}
        <button onclick={() => requestConfirm(r.id)} class="px-3 py-2 rounded-full bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold transition-colors cursor-pointer flex items-center gap-1.5">
...
<div class="space-y-6">
  <div>
    ...
    <p class="text-sm text-slate-500 mt-0.5">Von Lehrkräften angefragte Klassensätze — „Abschließen" gibt die geblockten Bestände wieder frei.</p>
  </div>
  ...
  {:else if offeneReservierungen.length === 0}
    <div class="py-16 text-center text-slate-400 text-base">Keine offenen Klassensatz-Reservierungen.</div>
  {:else}
