  ...
  {:else}
    ...
    <div class="space-y-3">
      ...
      <div class="space-y-1.5">
        ...
        <div class="relative">
          <input type="text" bind:value={labelStore.searchVal} oninput={labelStore.handleSearchInput} placeholder="Titel, Autor oder ISBN eingeben..." class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
        ...
        {#if labelStore.searchResults.length > 0}
          <div class="relative">
            <div class="absolute left-0 right-0 mt-1 bg-white border border-slate-100 rounded-xl shadow-xl z-20 max-h-48 overflow-y-auto divide-y divide-slate-50">
              {#each labelStore.searchResults as r}
                <button onclick={() => labelStore.selectBookTitle(r)} class="w-full text-left px-3.5 py-2.5 hover:bg-slate-50 transition-colors flex flex-col gap-0.5 cursor-pointer">
      ...
      <div class="grid grid-cols-2 gap-3">
        <div class="space-y-1.5">
          ...
          <select bind:value={labelStore.selectedClass} onchange={labelStore.handleClassChange} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-550">
            <option value="">-- Klasse wählen --</option>
            {#each labelStore.classGroups as group}
        ...
        <div class="space-y-1.5">
          ...
          <select disabled={!labelStore.selectedClass} onchange={(e) => {
            const bookId = /** @type {any} */ (e.target).value;
            const book = labelStore.classBooks.find((/** @type {any} */ b) => String(b.id) === bookId);
            if (book) {
            ...
            {#each labelStore.classBooks as book}
  ...
  {#if labelStore.selectedTitle}
    <div class="py-5 space-y-4 border-b border-gray-200">
      ...
      <div class="flex bg-slate-100 p-0.5 rounded-lg border border-slate-200/40 text-xs">
        <button onclick={() => labelStore.generationMode = "existing"} class="flex-1 text-center py-1 rounded-md font-bold transition-all cursor-pointer {labelStore.generationMode === 'existing' ? 'bg-white text-slate-800 shadow-xs' : 'text-slate-500 hover:text-slate-700'}">Vorhandene Exemplare</button>
        <button onclick={() => labelStore.generationMode = "new"} class="flex-1 text-center py-1 rounded-md font-bold transition-all cursor-pointer {labelStore.generationMode === 'new' ? 'bg-white text-slate-800 shadow-xs' : 'text-slate-500 hover:text-slate-700'}">Neue Barcodes</button>
      </div>

      {#if labelStore.generationMode === "existing"}
        <div class="space-y-2">
          <span class="text-[10px] uppercase font-bold text-slate-450 block">Exemplare auswählen ({labelStore.existingCopies.length} gefunden)</span>
          ...
          {:else}
            <div class="max-h-40 overflow-y-auto border border-slate-100 rounded-xl divide-y divide-slate-50 p-2 space-y-1 bg-slate-50/50">
              {#each labelStore.existingCopies as copy}
      ...
      {:else}
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="space-y-1.5">
            ...
            <input type="number" min="1" max="100" bind:value={labelStore.newQuantity} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-550" />
          </div>
          <div class="space-y-1.5">
            <span class="text-[10px] uppercase font-bold text-slate-450 block">Start-Ziffer (B-)</span>
            <input type="number" min="1" bind:value={labelStore.newStartNum} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-550" />
          </div>
        </div>
      {/if}
    </div>
  ...
  <div class="py-5 space-y-4 border-b border-gray-200">
    ...
    <div class="space-y-3.5">
      ...
      <div class="space-y-1.5">
        ...
        <select bind:value={labelStore.barcodeType} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 focus:outline-none">
      ...
      <label class="flex items-center space-x-3 text-xs text-slate-705 cursor-pointer select-none">
        <input type="checkbox" bind:checked={labelStore.labelBorder} class="accent-blue-600 w-4 h-4 rounded border-slate-200 bg-white" />
