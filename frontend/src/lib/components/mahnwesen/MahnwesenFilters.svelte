<script>
	...
	// Ref auf das Export-Menü, um es nach einer Auswahl zu schließen (natives <details>).
	let exportMenu = $state(/** @type {HTMLDetailsElement | null} */ (null));
...
<div class="flex items-center justify-between gap-4">
	...
	<!-- Rechts: erst Ansicht (Toggle), dann Aktionen — nur EINE Primär- und (kontextabhängig) EINE
	     Destruktiv-Aktion; alle Export-/Druck-Wege sammeln sich im „Export"-Menü. -->
	<div class="flex items-center gap-2 print:hidden shrink-0">
		...
		<!-- Export-Menü: sammelt Gesamt-PDF, Seitendruck und die Einzelklassen-PDF an EINER Stelle
		     mit eindeutigen Beschriftungen (löst die zwei mehrdeutigen „Drucken"-Buttons auf). -->
		<details bind:this={exportMenu} class="relative">
			<summary
				class="list-none [&::-webkit-details-marker]:hidden cursor-pointer select-none px-3 py-2 rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 text-xs font-bold flex items-center gap-1.5"
			>
				<svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path
						stroke-linecap="round"
			...
			<div
				class="absolute right-0 mt-2 w-72 bg-white border border-slate-200 rounded-2xl shadow-xl p-2 z-20"
			>
				<button
					onclick={() => {
				...
				<div class="px-2 pt-1 pb-1.5">
					...
					<div class="flex items-center gap-2">
						...
						<button
							onclick={() => {
								mahnwesenStore.downloadKlassePDF();
								closeExport();
							}}
		...
		<button
			...
			class="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-xs font-bold transition-all flex items-center gap-1.5 shadow-sm"
		>
		...
		<!-- Destruktive Massenaktion nur, wenn es überhaupt Fälle gibt: kein rotes „Alle anmahnen"
		     über einer leeren (🎉) Liste. -->
		{#if countAlle > 0}
			<button
				onclick={() => {
					if (
