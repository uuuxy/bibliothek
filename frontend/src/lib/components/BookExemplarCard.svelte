...
<div class="bg-white rounded-xl border p-4 shadow-sm transition-colors cursor-pointer {selected ? 'border-blue-500 bg-blue-50/30 ring-1 ring-blue-500' : 'border-slate-200 hover:border-slate-300'}"
  ...
  <div class="flex items-start justify-between mb-3">
        ...
        <div class="flex items-center gap-2">
          ...
          {#if appState.adminAuthenticated}
            {#if ex.barcode_id.startsWith('B-')}
              <a href={`/api/print/etikett/${ex.id}`} target="_blank" title="Ersatz-Etikett drucken" aria-label="Ersatz-Etikett drucken" class="text-slate-400 hover:text-blue-600 transition-colors cursor-pointer flex items-center gap-1" onclick={(e) => e.stopPropagation()}>
            ...
            {#if ex.barcode_id.startsWith('AUTO-') || ex.barcode_id.startsWith('SYS-')}
              ...
              >
                <svg aria-hidden="true" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                Barcode scannen
              </button>
            {:else}
              <button
                title="Barcode zuweisen/ändern"
                aria-label="Barcode zuweisen/ändern"
    ...
    <div class="flex items-center gap-2">
      ...
      {#if !editingStatus}
        <button title="Status ändern" aria-label="Status ändern" class="text-slate-400 hover:text-blue-600 transition-colors cursor-pointer" onclick={(e) => { e.stopPropagation(); editingStatus = true; }}>
          <svg aria-hidden="true" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
        </button>
        <button title="Exemplar löschen" aria-label="Exemplar löschen" class="text-slate-400 hover:text-rose-600 transition-colors cursor-pointer" onclick={(e) => { e.stopPropagation(); onDelete(); }}>
