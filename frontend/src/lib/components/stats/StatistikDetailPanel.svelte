...
<script>
  ...
  const systematikOptionen = $derived([...new Set(items.map((i) => i.systematik).filter(Boolean))].sort());
...
<div class="fixed inset-0 z-40 bg-slate-900/15" transition:fade={{ duration: 150 }} onclick={onClose} aria-hidden="true"></div>
...
>
  <header class="px-8 pt-8 pb-5 border-b border-slate-100 shrink-0 space-y-4">
    <div class="flex items-start justify-between gap-4">
      ...
      <button onclick={onClose} class="p-2 -m-2 text-slate-400 hover:text-slate-700 transition-colors cursor-pointer" aria-label="Detailansicht schließen">
    ...
    <div class="flex flex-wrap items-center gap-3">
      ...
      />
      <select bind:value={filterFach} class="px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm font-semibold text-slate-700" aria-label="Nach Fachbereich filtern">
        <option value="alle">Fachbereich: alle</option>
        {#each fachOptionen as f}<option value={f}>{f}</option>{/each}
      </select>
      <select bind:value={filterSystematik} class="px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm font-semibold text-slate-700" aria-label="Nach Systematik filtern">
        <option value="alle">Systematik: alle</option>
        {#each systematikOptionen as s}<option value={s}>{s}</option>{/each}
      </select>
    </div>
    <p class="text-[11px] text-slate-400 font-medium">{gefiltert.length} von {items.length} Einträgen</p>
  </header>
      ...
      <ul class="divide-y divide-slate-100">
        {#each gefiltert as row (row.id ?? row.titel + (row.isbn ?? ""))}
          <li class="px-8 py-3.5 flex items-center gap-4">
            {#if row.cover_url}
              <img src={row.cover_url} alt="" class="w-9 aspect-3/4 object-cover rounded-sm border border-slate-100 shrink-0" />
            {:else}
              <div class="w-9 aspect-3/4 bg-slate-50 border border-slate-100 rounded-sm flex items-center justify-center text-slate-300 text-xs shrink-0">📖</div>
            {/if}
