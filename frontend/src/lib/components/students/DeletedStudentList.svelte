...
<div class="bg-white rounded-2xl border border-rose-100 overflow-hidden shadow-xs w-full">
  ...
  {#if loadingDeleted}
    <div class="py-16 flex justify-center items-center">
      <div class="w-8 h-8 border-4 border-t-rose-600 border-slate-200 rounded-full animate-spin" aria-hidden="true"></div>
    </div>
  ...
  {:else}
    <div class="overflow-x-auto w-full text-left">
      <table class="w-full text-base text-slate-700">
        <thead class="bg-rose-50/50 border-b border-rose-100 uppercase tracking-wider text-sm font-bold text-slate-500 font-sans">
          <tr>
          ...
          {#each deletedStudents as s}
            <tr class="hover:bg-slate-50/50 transition-colors">
              ...
              <td class="px-6 py-3 text-sm text-slate-500">
                {new Date(s.deleted_at).toLocaleString('de-DE', {day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit'})}
              </td>
