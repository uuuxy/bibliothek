...
<div class="w-full max-w-2xl space-y-8">
	...
	{#if stage === 'done'}
		...
		<button
			onclick={resetFlow}
			class="px-5 py-2.5 rounded-full bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold transition-colors cursor-pointer"
		>
		...
		{#if stage === 'upload'}
			<div class="flex justify-end">
				<button
					...
					class="px-5 py-2.5 rounded-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white text-xs font-bold transition-all cursor-pointer flex items-center gap-2"
				>
					{#if previewLoading}
						<span
		...
		{#if stage === 'preview' && previewResult}
			<div class="space-y-4">
				...
				<div class="flex justify-end gap-3 pt-2">
					<button
						onclick={resetFlow}
						class="px-4 py-2.5 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-650 text-xs font-bold transition-colors cursor-pointer"
					>
						Andere Datei wählen
					</button>
