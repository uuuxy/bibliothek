    ...
    <div class="w-full space-y-6">
      ...
      <div class="flex items-center gap-4 bg-white p-4 rounded-2xl border border-slate-100 shadow-xs justify-between">
        <div class="flex flex-1 items-center gap-4">
          ...
          {#if role === 'admin' || role === 'mitarbeiter'}
            <button onclick={() => showCreateModal = true} class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-750 text-white font-bold py-2 px-4 rounded-xl text-sm transition-all shadow-sm cursor-pointer shrink-0">
          ...
          {#if role === 'admin'}
            <input type="file" accept=".csv" bind:this={fileInputEl} onchange={handleLUSDImport} class="hidden" />
            <button onclick={triggerImportPicker} class="inline-flex items-center gap-2 bg-slate-100 hover:bg-slate-200/80 text-slate-700 font-bold py-2 px-4 rounded-xl text-sm transition-all shadow-sm cursor-pointer shrink-0 border border-slate-200">
      ...
      {#if isImporting}
        <div class="p-4 bg-blue-55 border border-blue-100 rounded-2xl flex items-center justify-between text-xs font-semibold text-blue-700 animate-pulse">
          <div class="flex items-center gap-2">
            <div class="w-4 h-4 border-2 border-t-blue-600 border-blue-250 rounded-full animate-spin mr-1"></div>
      ...
      {#if importStatusMessage}
        <div class="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl flex items-center justify-between text-xs font-semibold text-emerald-800">
          ...
          <button onclick={() => importStatusMessage = ""} class="text-emerald-500 hover:text-emerald-700 font-bold text-sm bg-transparent border-none cursor-pointer">✕</button>
        </div>
      {/if}
      ...
      {#if importErrorMessage}
        <div class="p-4 bg-rose-50 border border-rose-100 rounded-2xl flex items-center justify-between text-xs font-semibold text-rose-800">
          ...
          <button onclick={() => importErrorMessage = ""} class="text-rose-500 hover:text-rose-750 font-bold text-sm bg-transparent border-none cursor-pointer">✕</button>
        </div>
      {/if}
        ...
        <label class="block text-xs font-bold uppercase tracking-wider text-slate-400">Klasse *
          <div class="mt-1.5 flex gap-2">
            {#if !customKlasseInput}
              <select bind:value={newKlasse} onchange={(e) => { if (e.target.value === "__custom__") { customKlasseInput = true; newKlasse = ""; } }} class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-slate-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all cursor-pointer font-sans">
