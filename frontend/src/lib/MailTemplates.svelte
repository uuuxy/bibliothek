...
<section class="w-full max-w-6xl mx-auto px-2 py-8 space-y-8">
    ...
    <div class="flex items-start justify-between gap-4 border-b border-gray-200 pb-6">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Passen Sie die Texte für Mahnungen und Bestellbenachrichtigungen an.</p>
        </div>
    ...
    <div class="flex flex-col lg:flex-row gap-10">
        ...
        <div class="lg:w-1/3 flex flex-col gap-2">
            {#if templates.length === 0}
                <div class="py-4 text-slate-500 text-sm text-center">
                    Lade Vorlagen...
                </div>
            {:else}
                {#each templates as t}
                    <button
                        class="text-left px-4 py-3 rounded-xl transition-all duration-200 border {selectedTemplateId === t.id ? 'bg-blue-50 border-blue-200' : 'border-transparent hover:bg-slate-50'}"
                    ...
                    >
                        ...
                        <div class="text-xs mt-0.5 truncate {selectedTemplateId === t.id ? 'text-blue-500' : 'text-slate-400'}">
                            {t.betreff}
                        </div>
                    </button>
                {/each}
        ...
        <div class="lg:w-2/3">
            {#if selectedTemplate}
                <div class="flex flex-col h-full gap-6">
                    ...
                    <div class="grow flex flex-col">
                        <label for="text_body" class="block text-sm font-medium text-gray-600 mb-2">Text-Inhalt</label>
                        <textarea
                            id="text_body"
            ...
            {:else if templates.length > 0}
                <div class="h-full flex flex-col items-center justify-center text-slate-400 p-8 border-2 border-dashed border-slate-200 rounded-2xl">
