...
<div class="max-w-3xl space-y-8">
  <div>
    ...
    <p class="text-xs text-slate-500 mt-1 leading-relaxed max-w-2xl">Ordnet jeder Klasse eine E-Mail-Adresse zu. Wird im Mahnwesen als Empfänger für Benachrichtigungen vorausgefüllt.</p>
  </div>
        ...
        {#each mappingRows as row}
          <tr class="hover:bg-slate-50/60 transition-colors">
            ...
            <td class="py-3 text-right">
              <button onclick={() => deleteMapping(row.klasse)} class="p-2 text-slate-400 hover:text-rose-600 rounded-lg transition-colors cursor-pointer" title="Mapping löschen">
  ...
  <div class="flex flex-col md:flex-row items-end gap-4">
    <div class="w-full md:w-32">
      <SettingField bind:value={newMappingKlasse} label="Klasse" type="text" placeholder="z.B. 7a" />
    </div>
    <div class="flex-1 w-full">
      <SettingField bind:value={newMappingEmail} label="E-Mail" type="email" placeholder="lehrkraft@schule.de" />
    </div>
    <button
