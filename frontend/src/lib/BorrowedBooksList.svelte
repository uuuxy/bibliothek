        ...
        <tr class="hover:bg-slate-50 transition-colors">
            ...
            <td class="py-3 px-4 text-sm font-semibold text-slate-700">
              {#if editingId === (book.ausleihe_id || book.id)}
                <div class="flex items-center gap-1.5">
                  ...
                  <button onclick={() => handleSaveDate(book)} disabled={isSavingDate} class="p-1 text-emerald-600 hover:bg-emerald-50 rounded disabled:opacity-50 transition-colors cursor-pointer" title="Speichern" aria-label="Rückgabedatum speichern">
                  ...
                  <button onclick={() => editingId = null} disabled={isSavingDate} class="p-1 text-rose-600 hover:bg-rose-50 rounded disabled:opacity-50 transition-colors cursor-pointer" title="Abbrechen" aria-label="Bearbeiten abbrechen">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"/></svg>
                  </button>
                </div>
              {:else}
                <div class="flex items-center gap-2 group">
                  <span>{new Date(book.rueckgabe_frist).toLocaleDateString("de-DE")}</span>
                  <button onclick={() => { editingId = book.ausleihe_id || book.id; editingDate = book.rueckgabe_frist.split('T')[0]; }} class="opacity-0 group-hover:opacity-100 p-0.5 text-gray-400 hover:text-blue-600 transition-opacity cursor-pointer" title="Datum bearbeiten" aria-label="Rückgabedatum bearbeiten">
          ...
          <td class="py-3 px-4 text-right">
            <div class="flex items-center justify-end gap-2">
              {#if mode === "loans"}
                <button onclick={() => handleExtend(book)} disabled={extendingIds.has(book.ausleihe_id || book.id)} class="p-2 bg-blue-50 hover:bg-blue-100 text-blue-600 disabled:opacity-50 rounded-full transition-colors cursor-pointer" title="Verlängern" aria-label="Ausleihe verlängern">
                  {#if extendingIds.has(book.ausleihe_id || book.id)}
                    <svg class="w-4 h-4 animate-spin text-blue-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"></path></svg>
                  {:else}
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  {/if}
                </button>
                {#if onDamageClick}
                  <button onclick={() => onDamageClick(book)} class="p-2 bg-rose-100 hover:bg-rose-200 text-rose-700 rounded-full transition-colors cursor-pointer" title="Verlust/Schaden melden" aria-label="Verlust oder Schaden melden">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                  </button>
                {/if}
                {#if onReturnClick}
                  <button onclick={() => onReturnClick(book.barcode_id)} class="p-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 rounded-full transition-colors cursor-pointer" title="Buch zurückgeben" aria-label="Buch zurückgeben">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/></svg>
                  </button>
                {/if}
              {:else if mode === "scans"}
