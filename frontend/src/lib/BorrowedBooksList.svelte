<script>
  /** @type {{ books: any[], onReturnClick?: (barcode: string) => void, mode?: "loans" | "scans" }} */
  let { books = [], onReturnClick = undefined, mode = "loans" } = $props();
</script>
  ...
  {#each books as book (book.id || book.barcode_id || Math.random())}
    ...
    <div class="p-1.5 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-all duration-200 flex flex-row items-center justify-between gap-3">
      <div class="flex items-center space-x-3 flex-1 min-w-0">
        ...
        {:else}
          <div class="w-6 h-9 rounded shadow-sm shrink-0 flex items-center justify-center font-bold text-white bg-gradient-to-br from-indigo-500 to-purple-600 text-[10px] border border-indigo-600/10">
