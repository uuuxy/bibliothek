  ...
  {#snippet children()}
    <div class="p-6 space-y-4">
      {#if duplicateConflict}
        <div class="p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3 text-sm font-semibold text-amber-800">
          ...
          <div>
            ...
            <p class="text-xs font-normal mt-1 opacity-80">Bitte überprüfe die Daten, um Duplikate zu vermeiden. Wurde der Schüler eventuell bereits angelegt oder importiert?</p>
          </div>
        </div>
      {/if}
      ...
      <label class="block text-xs font-bold uppercase tracking-wider text-slate-400">Geburtsdatum
        <input type="date" bind:value={newGeburtsdatum} class="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-slate-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-sans" />
      </label>
      ...
      <div class="flex justify-end gap-3 pt-2 border-t border-slate-100">
        <button onclick={handleClose} disabled={isSaving} class="rounded-xl bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-200 disabled:opacity-60 transition-colors cursor-pointer font-sans">Abbrechen</button>
