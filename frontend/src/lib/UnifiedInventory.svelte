...
<div class="max-w-4xl mx-auto w-full p-4 md:p-6 space-y-6 animate-fade-in">
  {#if inventoryState.status === 'idle'}
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-12 text-center flex flex-col items-center justify-center space-y-6">
      ...
      <button onclick={() => inventoryState.showStartModal = true} class="bg-blue-600 hover:bg-blue-700 text-white font-medium px-6 py-3 rounded-xl shadow-sm transition-colors flex items-center space-x-2">
  ...
  {:else}
    <div class="space-y-6">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <div class="flex justify-between items-end mb-4">
          <div>
            ...
            <div class="text-2xl font-bold text-slate-900 mt-1">{inventoryState.stats.erfasst} / {inventoryState.stats.erwartet} <span class="text-base font-medium text-slate-400">erfasst</span></div>
          </div>
          <div class="text-3xl font-bold text-blue-600">{inventoryState.getProgressPercent()}%</div>
        </div>
        <div class="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
          <div class="bg-blue-600 h-3 rounded-full transition-all duration-500 ease-out" style="width: {inventoryState.getProgressPercent()}%"></div>
        </div>
      </div>

      <!-- Scanner Input -->
      <form onsubmit={(e) => { e.preventDefault(); inventoryState.handleScan(inventoryState.barcodeInput, focusInput); }} class="relative">
      ...
      {#if inventoryState.lastScan}
        <div transition:slide class="rounded-2xl p-6 border {inventoryState.lastScan.success && inventoryState.lastScan.warnings.length === 0 ? 'bg-emerald-50 border-emerald-200' : (!inventoryState.lastScan.success ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200')}">
          <div class="flex items-start space-x-4">
            ...
            <div class="flex-1">
              <h4 class="text-lg font-bold {inventoryState.lastScan.success && inventoryState.lastScan.warnings.length === 0 ? 'text-emerald-900' : (!inventoryState.lastScan.success ? 'text-red-900' : 'text-amber-900')}">
                {inventoryState.lastScan.title}
              </h4>
              <p class="text-sm font-medium mt-1 {inventoryState.lastScan.success && inventoryState.lastScan.warnings.length === 0 ? 'text-emerald-700' : (!inventoryState.lastScan.success ? 'text-red-700' : 'text-amber-700')}">
                Barcode: {inventoryState.lastScan.barcode}
              </p>
              
              {#if inventoryState.lastScan.warnings.length > 0}
                <ul class="mt-3 space-y-1">
                  {#each inventoryState.lastScan.warnings as warn}
                    <li class="flex items-start text-sm {!inventoryState.lastScan.success ? 'text-red-800' : 'text-amber-800'}">
      ...
      <div class="pt-8 border-t border-slate-200 flex justify-end">
        <button onclick={() => inventoryState.showFinishModal = true} class="bg-red-50 hover:bg-red-100 text-red-600 font-semibold px-6 py-3 rounded-xl border border-red-200 transition-colors">
