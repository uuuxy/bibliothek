...
{#snippet roleToggle(item, roleLabel, roleKey)}
  ...
  <div class="flex items-center gap-3">
    ...
    >
      <input type="checkbox" checked={permissionsState[roleKey]?.[item.key] ?? false} class="sr-only peer" readonly />
    ...
    <div>
      ...
      <div class="divide-y divide-gray-200">
        {#each cat.items as item}
          <div class="py-6 px-1 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-slate-50/30 transition-colors">
            ...
            <div class="flex items-center gap-8 md:gap-12 shrink-0">
              <!-- Admin (Read-only) -->
              <div class="flex items-center gap-3">
                <span class="text-xs font-bold text-slate-400 tracking-wider w-16 text-right">ADMIN</span>
