...
<div class="w-full h-full text-slate-800 font-sans flex flex-col gap-6">
  ...
  <div class="flex items-end gap-6 border-b border-slate-200 shrink-0">
    {#snippet tab(id, label)}
      <button
        ...
        class="pb-3 text-sm font-semibold border-b-2 transition-colors cursor-pointer {activeTab === id ? 'border-blue-600 text-blue-700' : 'border-transparent text-slate-500 hover:text-slate-800'}"
      >{label}</button>
    {/snippet}
  ...
  {#if activeTab === "lieferanten"}
    <SupplierManager
      ...
      onAddSupplier={(name, email, customerNumber) => orderStore.addSupplier(name, email, customerNumber)}
      onEditSupplier={(id, name, email, customerNumber) => orderStore.editSupplier(id, name, email, customerNumber)}
