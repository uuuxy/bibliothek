...
<div class="min-h-screen bg-slate-50 flex flex-col">
  ...
  <header class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shadow-xs">
    ...
    <div class="text-xs text-emerald-600 font-semibold flex items-center gap-1.5 bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-100">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {#each results as book (book.id)}
          <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow flex flex-col">
            <!-- Cover area -->
            <div class="h-52 bg-linear-to-br from-slate-100 to-slate-200 flex items-center justify-center relative overflow-hidden">
              {#if book.cover_url}
              ...
              <div class="absolute top-2 right-2">
                {#if book.verfuegbar > 0}
                  <span class="px-2 py-1 rounded-lg bg-emerald-500 text-white text-xs font-bold shadow-sm">
                    ✓ Verfügbar
                  </span>
                {:else}
                  <span class="px-2 py-1 rounded-lg bg-rose-500 text-white text-xs font-bold shadow-sm">
                    Ausgeliehen
                  </span>
                {/if}
              </div>
