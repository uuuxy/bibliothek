		...
		if (!title || !title.id || title.id === '1') {
			copies = [
				{ id: 'e1', barcode_id: 'B-20031', zustand_notiz: 'Leichte Kratzer', ist_ausleihbar: true },
				{
					id: 'e2',
					barcode_id: 'B-20032',
					zustand_notiz: 'Eselsohren auf S. 12',
					ist_ausleihbar: true
				},
				{
					id: 'e3',
					barcode_id: 'B-20033',
					zustand_notiz: 'Unleserlicher Barcode',
					ist_ausleihbar: true
				},
...
{#if !title}
	<div class="py-12 flex flex-col items-center justify-center text-slate-400 space-y-2">
		...
		<span class="text-xs font-semibold"
			>Kein Buch ausgewählt. Bitte suche einen Buchtitel über die Ausleihe.</span
		>
	</div>
{:else}
		...
		>
			<div class="flex items-center space-x-4">
				{#if title.coverUrl}
					<img
						src={title.coverUrl}
						class="w-14 h-20 object-cover rounded-xl shadow-md border border-slate-100/50 shrink-0"
						alt="Cover"
					/>
				{:else}
					<div
						class="w-14 h-20 rounded-xl shadow-md shrink-0 flex items-center justify-center font-bold text-white bg-linear-to-br from-indigo-500 to-purple-650 text-lg border border-indigo-600/10"
					>
					...
					{#if title.erweiterteEigenschaften?.standort || title.signatur || title.erweiterteEigenschaften?.signatur}
						<div class="mt-1.5 flex flex-wrap gap-2">
							{#if title.erweiterteEigenschaften?.standort}
								<span
									class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-50 text-amber-800 border border-amber-100"
								>
			...
			<div
				class="text-sm bg-slate-50 border border-slate-100 rounded-2xl py-2 px-4 flex items-center gap-3"
			>
