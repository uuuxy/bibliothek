<script>
    ...
    async function loadExemplare() {
        ...
        error = "";
        try {
            const res = await apiFetch(`/api/buecher/titel/${formular.id}/exemplare`, { credentials: "include" });
            if (!res.ok) {
...
<div class="mt-8 border-t border-gray-100 pt-6">
    ...
    {:else if exemplare.length === 0}
        <div class="text-sm text-gray-500 py-4 italic text-center">Keine Exemplare in der Datenbank vorhanden. (Gesamtbestand: {formular.stock})</div>
    {:else}
        <div class="space-y-2 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
            {#each exemplare as ex}
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
                    <div class="flex items-center gap-3">
                        <span class="text-xs font-bold text-blue-700 bg-blue-50 border border-blue-100 px-2 py-1 rounded font-mono">
                            {ex.barcode_id}
                        </span>
                        ...
                        {#if ex.zustand_notiz}
                            <span class="text-[10px] text-gray-500 truncate max-w-[150px]" title={ex.zustand_notiz}>{ex.zustand_notiz}</span>
                        {/if}
                    </div>
                    <button title="Exemplar löschen" aria-label="Exemplar löschen" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors cursor-pointer" onclick={() => deleteCopy(ex)}>
