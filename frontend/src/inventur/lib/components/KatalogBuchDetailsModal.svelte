<script>
    ...
    async function ladeAusleiher() {
        ...
        try {
            ...
            console.error("Fehler beim Laden der Ausleiher:", e);
            borrowers = [];
        }
    }
...
>
    ...
    <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white z-10">
        ...
        <button
            onclick={onClose}
            class="p-2 hover:bg-slate-100 rounded-full text-slate-455 transition-colors cursor-pointer"
            aria-label="Schließen"
        >
    ...
    <div class="p-6 space-y-6 flex-1">
            ...
            <div class="flex-1 space-y-3">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4 text-sm text-slate-600 pt-2">
                    ...
                    <div>
                        ...
                        <span class="font-semibold text-slate-800">{book.verfuegbar} / {book.gesamt}</span>
                    </div>
                    <div>
                        ...
                        <div class="p-4 rounded-xl bg-slate-50/50 border border-slate-100 flex flex-col justify-between hover:border-slate-200 transition-colors shadow-xs">
                            <div class="flex items-start justify-between">
                                ...
                                <span class="text-[10px] font-bold px-2 py-0.5 rounded-full {
                                    !exemplar.ist_ausleihbar 
                                        ? 'bg-rose-50 text-rose-700 border border-rose-100' 
                                        : !exemplar.ist_verfuegbar 
        ...
        {#if borrowers && borrowers.length > 0}
            <div class="mt-8 border-t border-slate-100 pt-6">
                ...
                <div class="sticky top-0 pt-4 pb-3 z-10 bg-white flex flex-col gap-3 border-b border-slate-100 mb-3 -mx-2 px-2">
                                ...
                                {#each filteredBorrowers as b}
                                    <li class="p-3 hover:bg-slate-50 transition-colors flex items-center justify-between group">
                                        <div class="flex items-center gap-3 min-w-0">
                                            <div class="w-8 h-8 rounded-full bg-indigo-50 text-indigo-650 flex items-center justify-center font-bold text-xs shrink-0">
                                                {b.schueler_name ? b.schueler_name.charAt(0) : ''}{b.schueler_nachname ? b.schueler_nachname.charAt(0) : ''}
                                            </div>
                                            <div class="min-w-0">
                                                <button onclick={() => { appState.triggerStudentScan = b.schueler_barcode; onClose(); }} class="text-sm font-semibold text-slate-800 hover:text-indigo-600 text-left transition-colors cursor-pointer block truncate">
                                                    {b.schueler_name} {b.schueler_nachname} <span class="text-xs font-normal text-slate-500">({b.klasse || 'Unbekannt'})</span>
                                                </button>
                                                <div class="text-xs text-slate-400 mt-0.5 truncate font-medium">Exemplar: <span class="font-mono text-slate-500">{b.exemplar_barcode}</span></div>
                                            </div>
                                        </div>
