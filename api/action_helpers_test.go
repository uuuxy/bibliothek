...
func TestHandleStudentCheckoutFlow(t *testing.T) {
	...
	mock.ExpectQuery("SELECT schluessel, wert FROM system_einstellungen").
		WillReturnRows(pgxmock.NewRows([]string{"schluessel", "wert"}).
			...
			AddRow("standard_ausleihfrist_tage", "14"))
		// Check reservation
		mock.ExpectQuery("SELECT v.schueler_id, s.vorname, s.nachname FROM vormerkungen v JOIN schueler s ON v.schueler_id = s.id WHERE v.bereitgestellt_exemplar_id = \\$1 AND v.status = 'abholbereit' AND v.bereitgestellt_bis > CURRENT_TIMESTAMP").WithArgs(copy.ID).WillReturnRows(pgxmock.NewRows([]string{"schueler_id", "vorname", "nachname"}))
