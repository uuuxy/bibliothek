...
// je Schüler in stabiler Reihenfolge (Nachname, Vorname, Ausleihdatum).
func queryMahnungSchueler(ctx context.Context, dbPool db.PgxPoolIface, klasse string) ([]pdf.MahnungSchueler, error) {
	query := `
...
func PrintMahnungHandler(dbPool db.PgxPoolIface) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		schuelerListe, err := queryMahnungSchueler(ctx, dbPool, klasse)
		if err != nil {
			apierrors.SendHTTPError(w, http.StatusInternalServerError, err)
			return
		}

		if len(schuelerListe) == 0 {
