...
func (s *Server) ImportStudentsHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// ⚡ Bolt: Use a single UNNEST bulk UPSERT operation to eliminate the N+1 database bottleneck
		// when looping through rows, dramatically improving import performance for large CSVs.
		if count > 0 {
			upsertQuery := `
