...
func (s *OrderService) ProcessOrder(ctx context.Context, req SubmitOrderRequest) (*OrderResult, error) {
	...
	// ⚡ Bolt Optimization: Replaced O(n) individual tx.Exec INSERT statements
	// with a single bulk insert using pgx.CopyFrom. This leverages the PostgreSQL
	// COPY protocol, eliminating N+1 query overhead and significantly improving
	// throughput when processing large orders.
	if len(positionen) > 0 {
		var copyRows [][]any
