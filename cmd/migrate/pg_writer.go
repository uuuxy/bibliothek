...
func kuerzeFelder(el *uebernahme.Protokoll, m mysqlMedium) titelFelder {
	...
	f := titelFelder{
		Titel:      uebernahme.Kuerze(uebernahme.Kuerzung{Protokoll: el, QuellID: id, Kennung: isbn, Feld: "titel", Wert: m.Titel, Max: maxTitelSpalte}),
		Untertitel: uebernahme.KuerzeNullbar(uebernahme.Kuerzung{Protokoll: el, QuellID: id, Kennung: isbn, Feld: "untertitel", Wert: m.Untertitel.String, Max: maxTitelSpalte}),
		Autor:      uebernahme.KuerzeNullbar(uebernahme.Kuerzung{Protokoll: el, QuellID: id, Kennung: isbn, Feld: "autor", Wert: m.Autor.String, Max: maxTitelSpalte}),
		Verlag:     uebernahme.KuerzeNullbar(uebernahme.Kuerzung{Protokoll: el, QuellID: id, Kennung: isbn, Feld: "verlag", Wert: m.Verlag.String, Max: maxTitelSpalte}),
		Medientyp:  "Buch",
	}
	if m.Medientyp.Valid && m.Medientyp.String != "" {
		f.Medientyp = uebernahme.Kuerze(uebernahme.Kuerzung{Protokoll: el, QuellID: id, Kennung: isbn, Feld: "medientyp", Wert: m.Medientyp.String, Max: maxMedientypSpalte})
	}
	return f
}
