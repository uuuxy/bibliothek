...
func LoginHandler(dbPool db.PgxPoolIface, authenticator *Authenticator, cookieSecure bool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		user, ok := authenticateAndCheckStatus(ctx, w, dbPool, req.Email, password, bruteForceKey)
		if !ok {
			return
		}
...
// checkBruteForceLimit checks if the given email and IP combination is rate-limited.
// If it is, it writes the appropriate HTTP error response and returns false.
func checkBruteForceLimit(w http.ResponseWriter, email, clientIP string) (bruteForceKey string, ok bool) {
	...
	if globalLoginLimiter.isBlocked(key) {
...
// authenticateAndCheckStatus verifies credentials, handles errors, and checks user activation status.
// It returns the loaded user and true if successful, or false if an error response was already written.
func authenticateAndCheckStatus(ctx context.Context, w http.ResponseWriter, dbPool db.PgxPoolIface, email, password, bruteForceKey string) (loginUser, bool) {
