...
func LoginHandler(dbPool db.PgxPoolIface, authenticator *Authenticator, cookieSecure bool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		user, ok := authenticateUser(ctx, w, dbPool, req.Email, password, bruteForceKey)
		if !ok {
			return
		}
...
// checkBruteForceLimit bildet den Drosselschlüssel und weist ab, solange er gesperrt ist.
// ok=false: die Fehlerantwort wurde bereits geschrieben.
//
// Gedrosselt wird pro (E-Mail|IP) — NICHT rein pro IP. Sonst würde in einer Schule, in der
// alle Geräte hinter EINER NAT-IP hängen, ein einziger Nutzer mit 5 Fehlversuchen die
// GESAMTE Schule für 15 Minuten am Login hindern. Der zusammengesetzte Schlüssel sperrt
// nur das betroffene Konto auf dieser IP.
func checkBruteForceLimit(w http.ResponseWriter, email, clientIP string) (bruteForceKey string, ok bool) {
...
// authenticateUser prüft die Zugangsdaten und den Kontostatus. ok=false: die
// Fehlerantwort wurde bereits geschrieben.
//
// Ein Fehlversuch wird NUR beim falschen Passwort gezählt. Ausfall von Mailserver oder
// Datenbank und ein offener Antrag sind keine Fehlversuche — wer dort mit dem richtigen
// Passwort weiterprobiert, darf sich nicht selbst aussperren.
func authenticateUser(ctx context.Context, w http.ResponseWriter, dbPool db.PgxPoolIface, email, password, bruteForceKey string) (loginUser, bool) {
	user, verifyErr := verifyIMAPCredentials(ctx, dbPool, email, password)
	if verifyErr != nil {
