...
func LoginHandler(dbPool db.PgxPoolIface, authenticator *Authenticator, cookieSecure bool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		bruteForceKey, ok := checkBruteForce(w, req.Email, clientIP)
		if !ok {
			return
		}

		user, ok := validateAndFetchUser(ctx, w, dbPool, req.Email, password, bruteForceKey)
		if !ok {
			return
		}
...
// checkBruteForce prüft den Login-Limiter.
func checkBruteForce(w http.ResponseWriter, email, clientIP string) (bruteForceKey string, ok bool) {
...
// validateAndFetchUser validiert die Anmeldedaten und verarbeitet Fehler, die dabei auftreten.
func validateAndFetchUser(ctx context.Context, w http.ResponseWriter, dbPool db.PgxPoolIface, email, password, bruteForceKey string) (loginUser, bool) {
