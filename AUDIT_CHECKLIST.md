...
- [x] **Dockerfile/Compose** — Härtung umgesetzt. Korrektur: Dockerfile war bereits non-root (`appuser`) + hatte `HEALTHCHECK` (frühere Notiz war falsch). **Neu behoben:** (1) Code-Guard in `main.go/loadConfig` verweigert außerhalb von `APP_ENV=local` den Start mit den committeten Default-`JWT_SECRET`/`APP_ENCRYPTION_KEY` (sonst Admin-JWT-Fälschung / Foto-Entschlüsselung möglich) — gegen echte Env getestet. (2) `docker-compose.yml`: Default-Fallback-Secrets entfernt → Pflicht-Vars via `${VAR:?}` (JWT/AES/DB-Passwort), `COOKIE_SECURE` konfigurierbar, `APP_ENV=production`. (3) `docker-compose.local.yml`: `APP_ENV=local` + gültiges Dev-JWT (vorheriges 17-Zeichen-Default hätte den Längen-Check verletzt).
