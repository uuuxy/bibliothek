...
- [x] **Dockerfile/Compose** — Härtung umgesetzt. Korrektur: Dockerfile war bereits non-root (`appuser`) + hatte `HEALTHCHECK` (frühere Notiz war falsch). **Neu behoben:** (1) Code-Guard in `main.go/loadConfig` verweigert den Start mit den committeten Default-`JWT_SECRET`/`APP_ENCRYPTION_KEY` (sonst Admin-JWT-Fälschung / Foto-Entschlüsselung möglich) — gegen echte Env getestet. (2) `docker-compose.local.yml`: `APP_ENV=local` + gültiges Dev-JWT. **Nachgezogen (2026-06-25):** Der Guard wurde von `APP_ENV` **entkoppelt** und auf den dedizierten Schalter `ENFORCE_PROD_SECRETS` umgestellt — Begründung: `APP_ENV=local` hätte gleichzeitig Cookie-`Secure` deaktiviert und Swagger öffentlich freigeschaltet. Test-/Pilotphase läuft mit `ENFORCE_PROD_SECRETS=false` (Default, Stack startet mit Defaults); vor echtem Prod-Deploy `ENFORCE_PROD_SECRETS=true` + echte Secrets. `docker-compose.yml` nutzt jetzt bequeme `${VAR:-…}`-Defaults; die Prod-Absicherung übernimmt allein der Code-Guard.
