...
func TestHandleListExternalCovers(t *testing.T) {
	...
	queryRegex := `(?s)SELECT id, COALESCE\(isbn, ''\) AS isbn, titel AS title, COALESCE\(cover_url, ''\) AS cover_url.*FROM buecher_titel.*WHERE cover_url LIKE 'http%'.*ORDER BY id ASC.*LIMIT \$1`
